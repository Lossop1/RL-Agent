...
class _MockDriver:
    ...
    def apply_and_deploy(self, changes, note, stamp):
        self.deployed.append(changes)
        return f"/payload/{stamp}"
...
def _run(monkeypatch, scores, max_iters=1):
    ...
    monkeypatch.setattr(
        TO, "propose_change", lambda gaps, cur, hist: {"w_diagonal_contact": 0.55}
    )
    # avoid touching the real config/rollback file
    monkeypatch.setattr(
        "products.taili.blind_locomotion.taili_blind_config.load_taili_blind_config",
        lambda: {},
    )
    monkeypatch.setattr(
        "products.taili.blind_locomotion.taili_blind_config.get_config_value",
        lambda c, k: 0.45,
    )
    monkeypatch.setattr(
        "products.taili.ops.strategy_edit.rollback_last",
        lambda: rolled.append(1) or {"ok": True},
    )
    camp = TO.Campaign(max_iters=max_iters)
    summary = TO.run_campaign(
        camp,
        "run0",
        "agent_0.pt",
        log=lambda *a: None,
...
def test_campaign_resolves_bare_checkpoint_to_absolute_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "products.taili.ops.strategy_edit.rollback_last", lambda: {"ok": True}
    )
    TO.run_campaign(
        TO.Campaign(max_iters=1),
        "run0",
        "agent_15000.pt",
        log=lambda *a: None,
        stamp_fn=lambda: "s1",
    )
    assert drivers[0].launched_ckpts == [
        "/root/gpufree-data/taili_runs/run0/checkpoints/agent_15000.pt"
    ]
...
def test_campaign_stops_at_convergence(monkeypatch):
    ...
    def _run_noproposal(mp):
        ...
        return TO.run_campaign(
            TO.Campaign(max_iters=5), "r", "c.pt", log=lambda *a: None
        )

    summary = _run_noproposal(monkeypatch)
    assert (
        summary["n_iters"] == 0 and summary["best_score"] == 7
    )  # measured, found nothing productive, stopped
...
def test_campaign_stops_immediately_when_benchmark_passes(monkeypatch):
    ...
    summary = TO.run_campaign(
        TO.Campaign(max_iters=5), "r", "c.pt", log=lambda *a: None, stamp_fn=lambda: "s"
    )
