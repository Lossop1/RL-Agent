...
def _run(monkeypatch, scores, max_iters=1):
    ...
    monkeypatch.setattr("products.taili.blind_locomotion.taili_blind_config.load_taili_blind_config", lambda: {})
    monkeypatch.setattr("products.taili.blind_locomotion.taili_blind_config.get_config_value", lambda c, k: 0.45)
    monkeypatch.setattr("products.taili.ops.strategy_edit.rollback_last", lambda: rolled.append(1) or {"ok": True})
...
def test_campaign_resolves_bare_checkpoint_to_absolute_path(monkeypatch):
    ...
    monkeypatch.setattr("products.taili.blind_locomotion.taili_blind_config.load_taili_blind_config", lambda: {})
    monkeypatch.setattr("products.taili.blind_locomotion.taili_blind_config.get_config_value", lambda c, k: 0.45)
    monkeypatch.setattr("products.taili.ops.strategy_edit.rollback_last", lambda: {"ok": True})
...
def test_campaign_stops_at_convergence(monkeypatch):
    ...
    def _run_noproposal(mp):
        ...
        mp.setattr("products.taili.blind_locomotion.taili_blind_config.load_taili_blind_config", lambda: {})
        mp.setattr("products.taili.blind_locomotion.taili_blind_config.get_config_value", lambda c, k: 0.45)
