...
_VERDICT = {
    "gates": {
        ...
        "B3": {
            "ok": False,
            "detail": "swing peak clearance=0.221m ~0.08 (band .05-.15)",
        },
...
def test_propose_targets_worst_movable_gap():
    ...
    assert (
        "w_diagonal_contact" in ch and ch["w_diagonal_contact"] > 0.45
    )  # steps the symmetry lever
...
def test_regressed_lever_not_retried():
    ...
    ch = TO.propose_change(
        gaps, cur, [{"key": "w_diagonal_contact", "outcome": "regressed"}]
    )
...
def test_monitor_recovers_from_stall():
    class Mock(TO.TuningDriver):
        def __init__(self):
            self.seq = [100, 100, 100, 100, 200, 30001]
            self.i = 0
            self.restarts = 0
        ...
        def latest_checkpoint(self, r):
            return "/x/agent_100.pt"

        def launch(self, *a, **k):
            self.restarts += 1

    m = Mock()
    res = m.monitor_to_target(
        "r",
        30000,
        payload="p",
        checkpoint="c",
    ...
    assert (
        res["reached"] and m.restarts == 1
    )  # detected the stall, restarted once, reached
