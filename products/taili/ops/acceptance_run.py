"""在远程机上运行物理验收并按当前规范汇总 checkpoint 结果。

SSH 传输、评分器和汇总器是确定性组件；本命令只负责解析 run、启动 payload 内的
``physeval_blind``，再合并各地形结果。训练进程仍在运行时默认拒绝抢占资源，只有显式
``--force`` 才允许继续。

    python -m products.taili.ops.acceptance_run <run_id|newest> [--terrains flat rough ...]
...
from products.taili.blind_locomotion import acceptance_aggregate as AGG
...
def _maybe_update_best_registry(remote: RemoteSSH, data_root: str, verdict: dict) -> bool:
    ...
    try:
        ...
        # 门数量相同时，用总跌倒率作健康度次级排序；这样不会把同分但明显更安全的策略丢掉。
        fall = 0.0
        ...
        try:
            ...
            prev_score, prev_fall = -1, 1e9
        # 只有硬门更多，或硬门相同且跌倒率更低时，才更新 best 指针。
