...
_DOCS = _ROOT / "docs"
_TAILI_YAML = _ROOT / "products" / "taili" / "blind_locomotion" / "taili_blind_config.yaml"
...
def build_context_pack(query: str = "", include_docs: bool = True) -> dict[str, Any]:
    """Return this product's allowlisted strategy/spec context.

    This is deliberately local and read-only. It does not scan arbitrary paths; it reads the
    The product plugin owns its allowlist and strategy files; the system only consumes
    this role contract and never scans arbitrary paths.
    """
    ...
    return {
        "kind": "product_controlled_context_pack",
        "permission_boundary": (
                "Local read-only allowlist: selected Markdown paths under docs/ and "
            "products/taili/blind_locomotion/taili_blind_config.yaml. No arbitrary local scan, "
...
def build_taili_context_pack(query: str = "", include_docs: bool = True) -> dict[str, Any]:
    """兼容旧工具名；实现只有一份。"""
    return build_context_pack(query=query, include_docs=include_docs)
