...
它把验收、有限参数修改、远程启动和有界监控串成一个兼容流程。纯决策函数可在 CPU
上测试；远程循环必须经过资源和 checkpoint 保护。新的研究闭环应使用
``autotuner.research``，本模块只保留已有 CLI 能力。
"""
...
GATE_LEVERS: dict[str, list[tuple[str, float, float]]] = {
    "B3": [("w_clearance_over", 0.4, 3.0)],  # 抬脚不过量
    "B4": [("w_diagonal_contact", 0.1, 0.8), ("w_duty_balance", 0.1, 0.8)],  # 步态对称
    "F2": [("w_torque_margin", 0.15, 1.8)],  # 力矩裕量
    # yaw 使用已经验证过的转向风格通道；不把无效的单一跟踪权重无限放大。
    "A2": [("rew_imitate", 0.3, 3.0)],
    # 静止时四足落地由接触项负责；机身静止项不能替代支撑约束。
    "A3": [("w_stand_contact", 0.3, 3.0)],
    "C": [("w_stand_contact", 0.3, 3.0)],  # 与 A3 共享支撑问题
    "B1": [("w_landing_impact_late", 0.2, 1.6)],  # 落脚冲击
    "A1": [("w_wrong_dir", 0.1, 1.2)],  # 反向速度尾部
    # 楼梯主驱动是上向真实位移；感知和课程结构不在这里通过盲目增权解决。
    "D": [("w_climb", 0.3, 2.5)],
}
# 已确认是测量伪影的族不作为自动调参目标，避免把指标问题误改成策略问题。
SKIP_FAMILIES = {"B2"}
...
class GapAnalysis:
    ...
    margin: (
        float | None
    )  # how far over the spec (measured-threshold), None if unparseable
...
def _parse_stat(detail: str) -> tuple[float | None, float | None]:
    ...
    meas = None
    mm = re.search(
        r"(?:p90|p95|=|clearance=|slip\s+p90=|settle=)\s*([0-9]*\.?[0-9]+)", detail
    )
...
def analyze_gaps(verdict: dict[str, Any]) -> list[GapAnalysis]:
    ...
    out.sort(
        key=lambda g: (g.margin / g.threshold) if (g.margin and g.threshold) else -1,
        reverse=True,
    )
    return out
...
def propose_change(
    gaps: list[GapAnalysis], current: dict[str, float], history: list[dict[str, Any]]
) -> dict[str, float]:
    ...
    # 回退和应用失败的 lever 都加入排除集合，防止自动循环反复提交同一个无效候选。
    tried_and_failed = {
        h["key"] for h in history if h.get("outcome") in ("regressed", "apply_failed")
    }
    for gap in gaps:
...
class Campaign:
    goal: str = "benchmark"
    max_iters: int = 6
    steps_per_iter: int = 18000  # 单轮窗口上限，避免过训掩盖短期结果
    num_envs: int = 1024  # 默认资源规模；具体机器可在显式计划中覆盖
...
class TuningDriver:
    ...
    def apply_and_deploy(
        self, changes: dict[str, float], note: str, stamp: str
    ) -> str | None:
        """Apply the tuning to the local strategy, ship a fresh payload with it, return payload path."""
        import shlex
        from products.taili.ops.strategy_edit import apply_weight_changes
        ...
        base = self.remote.exec_out(
            f"ls -1td {self.data_root}/training_payloads/taili_blind_runtime_* | head -1"
        ).strip()
        ...
        for f in ("blind_tp_env.py", "physeval_blind.py", "acceptance_score.py"):
            sftp.put(
                f"products/taili/blind_locomotion/{f}", f"{new}/taili_blind_runtime/{f}"
            )
        sftp.put(
            "products/taili/blind_locomotion/acceptance_score.py",
            f"{new}/acceptance_score.py",
        )
        sftp.close()
        self.remote.exec(
            f"sed -i -E 's/^(\\s*init_phase:\\s*)[0-9]+/\\13/' "
            f"{shlex.quote(new)}/taili_blind_runtime/taili_blind_config.yaml",
            timeout=20,
        )
        self.log(
            f"[orchestrator] applied {res['applied']}, deployed {new.split('/')[-1]}"
        )
        return new
    ...
    def launch(self, payload: str, run_id: str, checkpoint: str, num_envs: int) -> None:
        ...
        self.remote.exec(
            "tmux kill-session -t taili_train 2>/dev/null; "
            "pkill -9 -f train_taili 2>/dev/null; sleep 4; true",
            timeout=30,
        )
        self.remote.exec("tmux new-session -d -s taili_train", timeout=15)
        cmd = (
            f"cd {self.data_root} && export PYTHONPATH={payload} && "
        ...
        self.remote.exec(
            f"tmux send-keys -t taili_train {shlex.quote(cmd)} Enter", timeout=15
        )
    ...
    def alive(self) -> bool:
        return bool(
            self.remote.exec_out(
                "pgrep -f train_taili | grep -v pgrep | head -1"
            ).strip()
        )

    def latest_checkpoint(self, run_id: str) -> str:
        return self.remote.exec_out(
            f"ls -1t {self.data_root}/taili_runs/{run_id}/checkpoints/agent_*.pt 2>/dev/null | head -1"
        ).strip()

    def monitor_to_target(
        self,
        run_id: str,
        target_step: int,
        *,
        ...
        while True:
            ...
            if (not alive) or frozen >= 3:  # death or ~3-poll stall
                ...
                self.log(
                    f"[orchestrator] STALL/death at {step}; restart {stalls}/{max_restarts} from {ck.split('/')[-1]}"
                )
                self.launch(payload, run_id, ck, num_envs)
                last, frozen = -1, 0

    def measure(
        self, run_id: str, checkpoint: str, terrains: list[str]
    ) -> dict[str, Any]:
        """Pause training and score the checkpoint vs the spec (physeval → §2 verdict)."""
        from products.taili.ops.acceptance_run import run_acceptance

        self.remote.exec(
            "tmux kill-session -t taili_train 2>/dev/null; "
            "pkill -9 -f train_taili 2>/dev/null; sleep 3; true",
            timeout=30,
        )
...
def run_campaign(
    ...
    *,
    ...
    from products.taili.ops.strategy_edit import rollback_last
    ...
    if from_checkpoint and not from_checkpoint.startswith("/"):
        from_checkpoint = (
            f"{d.data_root}/taili_runs/{from_run}/checkpoints/{from_checkpoint}"
        )
    history: list[dict[str, Any]] = []
    # 从研究台账恢复已回退的 lever，避免跨进程重复提交同一个候选；台账缺失时仍可运行兼容流程。
    try:
        from autotuner.research.campaign_journal import read_journal, record_iteration
    ...
    if read_journal:
        try:
            for pair in read_journal("taili").get("tried_and_rolled_back") or []:
                ...
                history.append(
                    {"key": (_lev.split()[0] if _lev else pair), "outcome": "regressed"}
                )
        except Exception:  # noqa: BLE001
            pass
    try:
        ...
        cur_ckpt = from_checkpoint
        for i in range(camp.max_iters):
            gaps = analyze_gaps(base)
            cfg = load_taili_blind_config()
            current = {
                k: get_config_value(cfg, f"reward.{k}")
                for _, lv in GATE_LEVERS.items()
                for k, *_ in lv
            }
            change = propose_change(
                gaps, {k: (v or 0.0) for k, v in current.items()}, history
            )
            if not change:
                log("[campaign] no productive lever remains — stopping")
                break
            stamp = stamp_fn()
            log(
                f"[campaign] iter {i + 1}: targeting {gaps[0].gate if gaps else '?'} with {change}"
            )
            payload = d.apply_and_deploy(
                change, note=f"campaign iter{i + 1}", stamp=stamp
            )
            if not payload:
                history.append({"key": list(change)[0], "outcome": "apply_failed"})
                continue
            run_id = f"campaign_{stamp}"
            d.launch(payload, run_id, cur_ckpt, camp.num_envs)
            mon = d.monitor_to_target(
                run_id,
                camp.steps_per_iter,
                payload=payload,
                checkpoint=cur_ckpt,
                num_envs=camp.num_envs,
                max_restarts=camp.max_stall_restarts,
            )
                ...
                camp.iterations.append(
                    {
                        ...
                        "score": score,
                        "reached": mon["reached"],
                        "checkpoint": mon["checkpoint"],
                        "benchmark_passed": True,
                    }
                )
                log(
                    f"[campaign] BENCHMARK PASSED at iter {i + 1} — stopping with the deliverable"
                )
                break
            improved = score > camp.best_score
            log(
                f"[campaign] iter {i + 1} score {score}/19 (best {camp.best_score}) reached={mon['reached']}"
            )
            history.append(
                {
                    "key": list(change)[0],
            ...
            if record_iteration:  # 将本轮决定写入持久台账
                try:
                    record_iteration(
                        "taili",
                        target_gate=(gaps[0].gate if gaps else ""),
                        lever=str(list(change)[0]),
                        decision=("kept" if improved else "rolled_back"),
                        score_before=f"{camp.best_score}/19",
                        score_after=f"{score}/19",
                        result_run=run_id,
                        config_diff=str(change),
                        note="run_campaign",
                    )
                except Exception:  # noqa: BLE001
                    pass
            ...
            if improved:
                camp.best_score, camp.best_checkpoint = score, mon["checkpoint"]
                base, cur_ckpt = verdict, mon["checkpoint"]
            else:
...
FULL_BATTERY = ["flat", "rough", "stairs", "stairs_up", "slope", "boxes", "push"]
...
def produce_policy(
    *,
    ...
    if not (from_run and from_checkpoint):
        # 没有显式输入时，从远端已登记的 best checkpoint 启动。
        from products.taili.ops.acceptance_run import _ssh_from_json
        ...
        if not ck:
            raise SystemExit(
                "no BEST_CHECKPOINT registry and no explicit run/checkpoint — "
                "run one acceptance measurement first to seed the registry"
            )
        parts = ck.split("/")
        from_run, from_checkpoint = parts[-3], parts[-1]
        log(
            f"[produce] bootstrapping from registry best: {from_run}/{from_checkpoint} "
    ...
    summary["benchmark_passed"] = any(
        it.get("benchmark_passed") for it in summary.get("iterations", [])
    )
    summary["deliverable"] = {
...
def main(argv=None) -> int:
    ...
    # 与 Campaign 默认值一致；自动路径不应无意中扩大单轮长度或环境数。
    ...
    if a.produce:
        summary = produce_policy(
            from_run=a.from_run if a.from_run != "auto" else "",
