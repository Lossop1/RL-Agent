...
class TuningDriver:
    ...
    def apply_and_deploy(self, changes: dict[str, float], note: str, stamp: str) -> str | None:
        ...
        sftp.put("products/taili/blind_locomotion/taili_blind_config.yaml",
                 f"{new}/taili_blind_runtime/taili_blind_config.yaml")
        for f in ("blind_tp_env.py", "physeval_blind.py", "acceptance_score.py"):
            sftp.put(f"products/taili/blind_locomotion/{f}", f"{new}/taili_blind_runtime/{f}")
        sftp.put("products/taili/blind_locomotion/acceptance_score.py", f"{new}/acceptance_score.py")
        sftp.close()
        self.remote.exec(
...
def run_campaign(camp: Campaign, from_run: str, from_checkpoint: str, *,
    ...
    not the process."""
    import time
    from products.taili.blind_locomotion.taili_blind_config import load_taili_blind_config, get_config_value
