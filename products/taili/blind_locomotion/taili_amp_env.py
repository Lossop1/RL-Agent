...
try:
    ...
    try:
        from products.taili.core.terrain_curriculum import (
...
try:
    ...
    try:
        from products.taili.core import taili_amp_reference, taili_geometry
    except ImportError:
        from taili_core import taili_amp_reference, taili_geometry
try:
    ...
    try:
        from products.taili.core.taili_reward import (
            transition_motion_fault,
            transition_support_phase_ready,
            update_transition_failure_ema,
        )
...
def _sample_range_value(n: int, lo: float, hi, device) -> torch.Tensor:
    if torch.is_tensor(hi):
        return torch.rand(n, device=device) * (hi - float(lo)).clamp(min=0.0) + float(
            lo
        )
...
def _sample_bucketed_commands(
    ...
    weights = torch.tensor(
        [max(0.0, _spec_float(raw, name, 0.0)) for name in names], device=device
    )
...
def _command_xy_world_from_root_yaw(
    root_quat_w: torch.Tensor, command_xy: torch.Tensor
) -> torch.Tensor:
...
class TailiAmpEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: TailiAmpEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._motion_loader = MultiMotionLoader(
            ...
            device=self.device,
            sigma=self.cfg.cond_sigma,
            slope_sigma=self.cfg.slope_sigma,
        )
        self.ref_body_index = self.robot.data.body_names.index(self.cfg.reference_body)
        self.foot_indexes = [
            self.robot.data.body_names.index(n) for n in self.cfg.foot_body_names
        ]
        ...
        self.motion_foot_indexes = self._motion_loader.get_body_index(
            self.cfg.foot_body_names
        )
        ...
        if self._amp_frame_stride > 1:
            self._amp_raw_depth = (
                self.cfg.num_amp_observations - 1
            ) * self._amp_frame_stride + 1
            self._amp_raw_ring = torch.zeros(
                (self.num_envs, self._amp_raw_depth, self.cfg.amp_observation_space),
                device=self.device,
            )
        ...
        self._cmd_target = torch.zeros(
            (self.num_envs, 3), device=self.device
        )  # 当前用户目标命令。
        self._cmd_previous = torch.zeros_like(self.commands)  # 最近一次切换前的命令。
        self._cmd_age_s = torch.zeros(
            self.num_envs, device=self.device
        )  # 当前命令持续时间。
        self._cmd_motion_target = torch.zeros_like(
            self.commands
        )  # 过渡整形后的目标命令。
        self._cmd_transition_start = torch.zeros_like(self.commands)
        self._cmd_transition_goal = torch.zeros_like(self.commands)
        self._cmd_transition_timer = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_total = torch.ones(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_via_zero = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_zero_frac = torch.full(
            (self.num_envs,), 0.5, device=self.device
        )
        self._cmd_transition_strength = torch.zeros(self.num_envs, device=self.device)
        self._cmd_transition_wait_steps = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_stable_steps = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_state = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_require_stop = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        # 0=无过渡，1=停车，2=反向，3=换轴。分类只用于评价语义，不改写用户命令。
        self._cmd_transition_kind = torch.zeros(
            self.num_envs, dtype=torch.int8, device=self.device
        )
        self._cmd_transition_old_axis = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_stage_step = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_elapsed_steps = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_decel_steps = torch.ones(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_accel_steps = torch.ones(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self._cmd_transition_action_rate = torch.zeros(
            self.num_envs, device=self.device
        )
        self._cmd_transition_strict_ready = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_body_safe = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_motion_ready = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_posture_ready = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_support_phase_ready = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_motion_fault = torch.zeros(
            self.num_envs, device=self.device
        )
        self._cmd_transition_initial_motion_fault = torch.zeros(
            self.num_envs, device=self.device
        )
        self._cmd_transition_motion_target = torch.zeros(
            self.num_envs, device=self.device
        )
        self._cmd_transition_motion_excess = torch.zeros(
            self.num_envs, device=self.device
        )
        self._cmd_transition_support_ema = torch.full(
            (self.num_envs,), 4.0, device=self.device
        )
        self._cmd_transition_failed = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_transition_failed_event = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        ...
        self._cmd_transition_flat = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        ...
        self._cmd_transition_flat_kind_event_count = {
            "stop": 0,
            "reverse": 0,
            "axis": 0,
        }
        self._cmd_transition_flat_kind_failure_count = {
            "stop": 0,
            "reverse": 0,
            "axis": 0,
        }
        self._cmd_transition_flat_kind_pending_event_count = {
            "stop": 0,
            "reverse": 0,
            "axis": 0,
        }
        self._cmd_transition_flat_kind_pending_failure_count = {
            "stop": 0,
            "reverse": 0,
            "axis": 0,
        }
        self._cmd_transition_flat_kind_failure_ema = {
            "stop": 0.0,
            "reverse": 0.0,
            "axis": 0.0,
        }
        self._cmd_transition_flat_kind_failure_ema_valid = {
            "stop": False,
            "reverse": False,
            "axis": False,
        }
        # handoff 只评价安全制动后新方向是否及时接管，不改写命令或动作。
        self._cmd_handoff_active = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._cmd_handoff_timer = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        ...
        self._command_execution_valid = torch.zeros(
            4, dtype=torch.bool, device=self.device
        )
        self._command_execution_coverage = {
            "fwd": 1.0,
            "back": 1.0,
            "lat": 1.0,
            "yaw": 1.0,
        ...
        self._cmd_resample_steps = max(
            1, int(self.cfg.cmd_resample_s / (self.cfg.dt * self.cfg.decimation))
        )
        self._log_step = 0  # 周期性 stdout 训练诊断计数。
        self._terrain_ctx = torch.zeros(
            (self.num_envs, self.cfg.terrain_ctx_dim), device=self.device
        )
        ...
        self._trot_offsets = torch.tensor(
            [0.0, 0.5, 0.5, 0.0], device=self.device
        )  # FL,FR,RL,RR 对角小跑。
        self._hip_idx = [
            self.robot.data.joint_names.index(f"{lg}_hip_joint")
            for lg in ["FL", "FR", "RL", "RR"]
        ]

        def _joint_gain_vector(
            attr_name: str, defaults: dict[str, float]
        ) -> torch.Tensor:
        ...
        self._vel_curriculum_enable = bool(
            getattr(self.cfg, "vel_curriculum_enable", True)
        )
        phase0_spec = (
            phase_command_spec(self.cfg, 0) if phase_command_spec is not None else {}
        )
        ...
        self._vel_max_fwd = (
            float(self.cfg.cmd_fwd_max)
            if not self._vel_curriculum_enable
            else _phase_hi("fwd_range", self.cfg.cmd_fwd_range)
        )
        self._vel_max_back = (
            float(self.cfg.cmd_back_max)
            if not self._vel_curriculum_enable
            else _phase_hi("back_range", self.cfg.cmd_back_range)
        )
        self._vel_max_lat = (
            float(self.cfg.cmd_lat_max)
            if not self._vel_curriculum_enable
            else _phase_hi("lat_range", self.cfg.cmd_lat_range)
        )
        ...
        self._vel_max_yaw = (
            float(self.cfg.cmd_yaw_max)
            if not self._vel_curriculum_enable
            else min(yaw_hi, yaw_lo + 0.20)
        )
        ...
        self._lat_prog = 0.0
        self._yaw_prog = 0.0
        self._progress_ema_initialized = {
            name: False for name in ("fwd", "back", "lat", "yaw")
        }
        ...
        self._dr_level = int(
            getattr(self.cfg, "dr_start_level", 0)
        )  # 按门控从 0 升到 3。
        ...
        self._phase = int(
            os.environ.get("TAILI_INIT_PHASE", getattr(self.cfg, "init_phase", 0))
        )
        self._phase_count = 0  # 连续满足当前阶段推进门槛的日志间隔数。
        self._penalty_budget_controls_quality = bool(
            getattr(self.cfg, "penalty_budget_controls_quality", True)
        )
        self._penalty_gate = (
            1.0
            if self._phase >= 1 or not self._penalty_budget_controls_quality
            else 0.0
        )
        ...
        self._dr_start_phase = int(
            getattr(self.cfg, "dr_start_phase", self._terrain_start_phase)
        )
        configured_max_phase = getattr(self.cfg, "max_training_phase", None)
        if configured_max_phase is None:
            try:
                configured_max_phase = max(
                    int(k)
                    for k in getattr(self.cfg, "training_phase_commands", {}).keys()
                )
            except Exception:
        ...
        if self._phase > 0:
            print(
                f"[ENV] init_phase={self._phase} (resume in trained regime; penalty_gate={self._penalty_gate})",
                flush=True,
            )
    ...
    def _build_hscan_mirror_perm(self):
        ...
        try:
            ...
            rs = (
                rs.reshape(-1, rs.shape[-1])[:P].detach().cpu().numpy()
            )  # (P,3) local ray grid
            xy = rs[:, :2]
            ...
            if (
                np.array_equal(perm[perm], np.arange(P))
                and len(set(perm.tolist())) == P
            ):
                return perm
            print(
                "[SYM] height_scan mirror not a clean involution; using identity",
                flush=True,
            )
        except Exception as e:
            print(
                f"[SYM] height_scan ray layout unavailable ({e}); using identity",
                flush=True,
            )
    ...
    def _begin_command_transition(self, env_ids, snap: bool = False):
        ...
        self._cmd_age_s[env_ids] = (
            float(getattr(self.cfg, "cmd_transition_max_s", 0.35)) if snap else 0.0
        )
        ...
        actual_lin = torch.linalg.norm(
            self.robot.data.root_lin_vel_b[env_ids, :2], dim=-1
        )
        low_speed_ready = (
            (actual_lin <= low_v)
            & (gyro_b[:, 2].abs() <= low_w)
            & (
                torch.linalg.norm(gyro_b[:, :2], dim=-1)
                <= float(getattr(self.cfg, "cmd_transition_wxy_max", 0.25))
            )
            & (
                joint_speed
                <= float(getattr(self.cfg, "cmd_transition_joint_speed_max", 1.5))
            )
            & upright_ready
        )
        unsafe_restart = (
            tgt_move & (self._cmd_transition_state[env_ids] > 0) & ~low_speed_ready
        )
        abrupt = (
            (sign_flip | family_change | through_stop) & ~low_speed_ready
        ) | unsafe_restart
        direct = ~abrupt
        ...
        if bool(abrupt.any()):
            ...
            delta_lin = torch.linalg.norm((target - current)[:, :2], dim=-1) / max(
                max_v, 1e-6
            )
            ...
            speed_norm = torch.maximum(
                ...
                torch.maximum(current[:, 2].abs(), gyro_b[:, 2].abs())
                / max(low_w, 1e-6),
            )
            ...
            max_steps = max(
                ...
                int(
                    round(
                        float(getattr(self.cfg, "cmd_transition_max_s", 0.35)) / step_dt
                    )
                ),
            )
            steps = (
                torch.round(
                    float(fast_steps)
                    + (float(base_steps) - float(fast_steps)) * severity[idx]
                )
                .long()
                .clamp(min=fast_steps, max=max_steps)
            )
            ...
            zero_frac_all = torch.where(
                tgt_move, zero_frac_all, torch.full_like(zero_frac_all, stop_z)
            )
            self._cmd_transition_start[ids] = self.commands[ids]
            self._cmd_transition_goal[ids] = self._cmd_target[ids]
            ...
            self._cmd_transition_stable_steps[ids] = 0
            decel_steps = (
                torch.round(steps.float() * zero_frac_all[idx]).long().clamp(min=1)
            )
            ...
            self._cmd_transition_motion_fault[ids] = (
                self._cmd_transition_initial_motion_fault[ids]
            )
            self._cmd_transition_motion_target[ids] = (
                self._cmd_transition_initial_motion_fault[ids]
            )
            ...
            if contact is not None:
                self._cmd_transition_support_ema[ids] = (
                    (contact[ids] > 0.5).float().sum(dim=-1)
                )
    ...
    def _flush_transition_rate_stats(self):
        ...
        beta = min(
            max(float(getattr(self.cfg, "cmd_transition_failure_ema_beta", 0.95)), 0.0),
            0.999,
        )

        self._cmd_transition_failure_ema, self._cmd_transition_failure_ema_valid = (
            update_transition_failure_ema(
        ...
        self._cmd_handoff_failure_ema, self._cmd_handoff_failure_ema_valid = (
            update_transition_failure_ema(
    ...
    def _update_command_handoff(self):
        ...
        lin_ready = (~moving_lin) | (
            lin_along >= torch.maximum(0.20 * lin_goal, torch.full_like(lin_goal, 0.05))
        )
        yaw_ready = (~moving_yaw) | (
            yaw_along >= torch.maximum(0.20 * yaw_goal, torch.full_like(yaw_goal, 0.04))
        )
        stop_ready = (
            torch.linalg.norm(vel, dim=-1)
            <= float(getattr(self.cfg, "cmd_transition_low_speed_v", 0.10))
        ) & (
        ...
        ready = body_ready & torch.where(
            target_still, stop_ready, lin_ready & yaw_ready
        )
        ready_ids = ids[ready]
        if len(ready_ids) > 0:
        ...
        max_steps = max(
            ...
            int(
                round(
                    float(getattr(self.cfg, "cmd_transition_handoff_s", 0.40)) / step_dt
                )
            ),
        )
    ...
    def _update_command_transition(self):
        ...
        if bool(decel.any()):
            ...
            p = ((self._cmd_transition_stage_step[ids].float() + 1.0) / total).clamp(
                0.0, 1.0
            )
            s = _smoothstep5(p)
            self._cmd_motion_target[ids] = self._cmd_transition_start[ids] * (
                1.0 - s[:, None]
            )
            self._cmd_transition_stage_step[ids] += 1
            done = (
                self._cmd_transition_stage_step[ids]
                >= self._cmd_transition_decel_steps[ids]
            )
            settle_ids = ids[done]
            if len(settle_ids) > 0:
        ...
        if bool(reanchor.any()):
            ...
            self._cmd_transition_accel_steps[ids] = max(
                1, int(round(release_s / step_dt))
            )
        ...
        if bool(accel.any()):
            ...
            self._cmd_transition_stage_step[ids] += 1
                ...
                self._cmd_transition_stage_step[ids]
                >= self._cmd_transition_accel_steps[ids]
            )
            done_ids = ids[done]
            if len(done_ids) > 0:
                ...
                if len(flat_done) > 0:
                    ...
                    self._cmd_handoff_goal[flat_done] = self._cmd_transition_goal[
                        flat_done
                    ]
    ...
    def _evaluate_command_transition_post_physics(self):
        ...
        self._cmd_transition_action_rate[ids] = (
            (self.actions[ids] - self.last_actions[ids]).abs().amax(dim=-1)
        )
        ...
        max_total_steps = max(
            1,
            int(
                round(float(getattr(self.cfg, "cmd_transition_max_s", 0.35)) / step_dt)
            ),
        )
        tau = (
            self._cmd_transition_elapsed_steps[ids].float() / float(max_total_steps)
        ).clamp(0.0, 1.0)
        smooth_tau = tau.pow(3) * (10.0 + tau * (-15.0 + 6.0 * tau))
        motion_target = self._cmd_transition_initial_motion_fault[ids] * (
            1.0 - smooth_tau
        )
        self._cmd_transition_motion_fault[ids] = motion_fault
        self._cmd_transition_motion_target[ids] = motion_target
        self._cmd_transition_motion_excess[ids] = torch.clamp(
            motion_fault - motion_target, min=0.0
        )

        wxy_limit = float(getattr(self.cfg, "cmd_transition_wxy_max", 0.35))
        tilt_deg = float(getattr(self.cfg, "cmd_transition_tilt_deg", 8.0))
        posture_ready = (wxy <= wxy_limit) & (tilt <= math.sin(math.radians(tilt_deg)))
        contact = getattr(
        ...
        stop_stable_steps = max(
            ...
            int(
                round(
                    float(getattr(self.cfg, "cmd_transition_stable_s", 0.12)) / step_dt
                )
            ),
        )
        ...
        required_steps = torch.where(
            ...
            torch.full_like(
                self._cmd_transition_stable_steps[ids], redirect_stable_steps
            ),
        )
    ...
    def _update_heading_error(self):
        ...
        yaw_active = (self.commands[:, 2].abs() > 0.05) | (
            self._cmd_target[:, 2].abs() > 0.05
        )
    ...
    def _update_command_execution_coverage(self):
        ...
        for index, (name, mask, ok) in enumerate(zip(names, masks, success)):
            ...
            if bool(self._command_execution_valid[index]):
                self._command_execution_ema[index] = (
                    beta * self._command_execution_ema[index] + (1.0 - beta) * sample
                )
            else:
                self._command_execution_ema[index] = sample
                self._command_execution_valid[index] = True
            self._command_execution_coverage[name] = float(
                self._command_execution_ema[index]
            )
    ...
    def _resample_commands(self, env_ids, snap=False):
        ...
        spec = (
            phase_command_spec(self.cfg, phase)
            if phase_command_spec is not None
            else {}
        )
        mode = str(
            spec.get("command_mode")
            or getattr(self.cfg, "training_command_mode", "normal")
            or "normal"
        )
        ...
        if mode in {
            ...
            def _range_with_ceiling(
                name: str, fallback, ceiling_attr: str, max_attr: str
            ):
                lo, hi = _spec_range(spec, name, fallback)
                hi = min(
                    hi,
                    float(getattr(self, ceiling_attr, hi)),
                    float(getattr(self.cfg, max_attr, hi)),
                )
                return lo, max(lo, hi)

            f_lo, f_hi = _range_with_ceiling(
                "fwd_range", self.cfg.cmd_fwd_range, "_vel_max_fwd", "cmd_fwd_max"
            )
            b_lo, b_hi = _range_with_ceiling(
                "back_range", self.cfg.cmd_back_range, "_vel_max_back", "cmd_back_max"
            )
            l_lo, l_hi = _range_with_ceiling(
                "lat_range", self.cfg.cmd_lat_range, "_vel_max_lat", "cmd_lat_max"
            )
            y_lo, y_hi = _range_with_ceiling(
                "yaw_range", self.cfg.cmd_yaw_range, "_vel_max_yaw", "cmd_yaw_max"
            )

            terr_frac = None
            if (
                self.cfg.vel_terrain_decouple
                and self.cfg.terrain.terrain_type == "generator"
            ):
                tmax = max(1.0, self.cfg.terrain.terrain_generator.num_rows - 1)
                terr_frac = (
                    self._terrain.terrain_levels[env_ids].float() / tmax
                ).clamp(0.0, 1.0)
            ...
            if mode == "fixed_forward":
                target[:, 0] = _spec_float(
                    spec, "fixed_vx", float(getattr(self.cfg, "training_fixed_vx", 0.5))
                )
            elif mode == "forward_range":
                lo_v, hi_v = _spec_range(
                    spec,
                    "forward_range",
                    getattr(self.cfg, "training_forward_range", (0.3, 0.7)),
                )
                ...
                if bucketed is not None:
                    ...
                    stand_prob = _spec_float(
                        spec, "stand_prob", float(getattr(self.cfg, "stand_prob", 0.0))
                    )
                    ...
                    p_fwd = max(0.0, _spec_float(spec, "prob_fwd", 0.5))
                    p_back = max(0.0, _spec_float(spec, "prob_back", 0.5))
                    x_is_fwd = torch.rand(n, device=dev) < (
                        p_fwd / max(1e-9, p_fwd + p_back)
                    )
                    x_on = torch.rand(n, device=dev) < _spec_float(
                        spec, "x_axis_prob", 1.0
                    )
                    lat_on = torch.rand(n, device=dev) < axis_prob
                    yaw_on = torch.rand(n, device=dev) < axis_prob
                    yaw_on = yaw_on | (~x_on & ~lat_on)
                    lat_sign = torch.where(
                        torch.rand(n, device=dev) < 0.5,
                    ...
                    target[:, 0] = torch.where(
                        x_on, target[:, 0], torch.zeros(n, device=dev)
                    )
                    ...
                    target = torch.where(
                        moving[:, None], target, torch.zeros_like(target)
                    )
                    nz_scale = _spec_float(spec, "near_zero_scale", 0.05)
                    ...
                    [
                        max(
                            ...
                            _spec_float(
                                spec,
                                "prob_fwd",
                                float(getattr(self.cfg, "cmd_prob_fwd", 0.25)),
                            ),
                        ),
                        max(
                            0.0,
                            _spec_float(
                                spec,
                                "prob_back",
                                float(getattr(self.cfg, "cmd_prob_back", 0.25)),
                            ),
                        ),
                        max(
                            0.0,
                            _spec_float(
                                spec,
                                "prob_lat",
                                float(getattr(self.cfg, "cmd_prob_lat", 0.25)),
                            ),
                        ),
                        max(
                            0.0,
                            _spec_float(
                                spec,
                                "prob_yaw",
                                float(getattr(self.cfg, "cmd_prob_yaw", 0.25)),
                            ),
                        ),
                    ],
                    device=dev,
        ...
        if (
            ...
            tmax = max(1.0, self.cfg.terrain.terrain_generator.num_rows - 1)
            _terr_frac = (self._terrain.terrain_levels[env_ids].float() / tmax).clamp(
                0.0, 1.0
            )
            fwd_ceil = self.cfg.cmd_fwd_range[0] + (
                self._vel_max_fwd - self.cfg.cmd_fwd_range[0]
            ) * (1.0 - _terr_frac)
        fwd_cmd = (
            torch.rand(n, device=dev) * (fwd_ceil - self.cfg.cmd_fwd_range[0])
            + self.cfg.cmd_fwd_range[0]
        )
        # 后退：与前进一样地形解耦，避免下台阶后退时速度过高导致力矩饱和。
        back_hi = max(
            self.cfg.cmd_back_range[0], min(self._vel_max_back, self.cfg.cmd_back_max)
        )
        if _terr_frac is not None:
            back_hi = self.cfg.cmd_back_range[0] + (
                back_hi - self.cfg.cmd_back_range[0]
            ) * (1.0 - _terr_frac)
        back_cmd = (
            torch.rand(n, device=dev) * (back_hi - self.cfg.cmd_back_range[0])
            + self.cfg.cmd_back_range[0]
        )
        self._cmd_target[env_ids, 0] = torch.where(
            fwd, fwd_cmd, torch.where(back, -back_cmd, torch.zeros(n, device=dev))
        )
        # 横移速度课程上限。
        lat_hi = max(
            self.cfg.cmd_lat_range[0], min(self._vel_max_lat, self.cfg.cmd_lat_max)
        )
        lat_cmd = (
            torch.rand(n, device=dev) * (lat_hi - self.cfg.cmd_lat_range[0])
            + self.cfg.cmd_lat_range[0]
        )
        self._cmd_target[env_ids, 1] = torch.where(
            lat,
            lat_cmd * torch.where(torch.rand(n, device=dev) < 0.5, 1.0, -1.0),
            torch.zeros(n, device=dev),
        )
        # yaw 速度课程上限。
        yaw_hi = max(
            self.cfg.cmd_yaw_range[0], min(self._vel_max_yaw, self.cfg.cmd_yaw_max)
        )
        yaw_cmd = (
            torch.rand(n, device=dev) * (yaw_hi - self.cfg.cmd_yaw_range[0])
            + self.cfg.cmd_yaw_range[0]
        )
        self._cmd_target[env_ids, 2] = torch.where(
            yaw,
            yaw_cmd * torch.where(torch.rand(n, device=dev) < 0.5, 1.0, -1.0),
            torch.zeros(n, device=dev),
        )
        self._begin_command_transition(env_ids, snap=snap)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        target_changed = (self._cmd_target - self._cmd_transition_goal).abs().amax(
            dim=1
        ) > 1e-6
        if bool(target_changed.any()):
            self._begin_command_transition(
                target_changed.nonzero(as_tuple=False).flatten(), snap=False
            )
        ...
        if bool(getattr(self.cfg, "cmd_transition_policy_managed", False)):
            ...
            if bool(transition_active.any()):
                self.commands[transition_active] = self._cmd_motion_target[
                    transition_active
                ]
        ...
        moving = (spd > 0.1) | (
            phase_command[:, 2].abs() > 0.05
        )  # 与奖励侧 yaw_cmd_gate 对齐。
        ...
        period = torch.clamp(
            self.cfg.gait_period - self.cfg.gait_period_slope * period_speed,
            min=self.cfg.gait_period_min,
            max=self.cfg.gait_period,
        )
        self._gait_phase = (
            self._gait_phase + (step_dt / period) * moving.float()
        ) % 1.0
        # 域随机化：周期性给 base 随机推力速度，训练扰动恢复能力。
        if self.cfg.dr_enable:
            due = (self.episode_length_buf % self._push_steps == 0) & (
                self.episode_length_buf > 0
            )
            if bool(due.any()):
                ids = due.nonzero(as_tuple=False).flatten()
                push_vel = float(
                    getattr(self.cfg, f"dr_push_vel_{self._dr_level}", 0.0)
                )
                if push_vel <= 0.0:
                    return
                vel = torch.cat(
                    [
                        self.robot.data.root_lin_vel_w[ids],
                        self.robot.data.root_ang_vel_w[ids],
                    ],
                    dim=-1,
                ).clone()  # (M, 6)，世界系线速度+角速度。
                vel[:, 0:2] += (
                    torch.rand((len(ids), 2), device=self.device) * 2 - 1
                ) * push_vel  # 线速度推扰。
                # 同时加入 yaw 角速度扰动，使策略学习旋转扰动恢复。
                vel[:, 5] += (
                    (torch.rand(len(ids), device=self.device) * 2 - 1)
                    * push_vel
                    * self.cfg.dr_push_ang_scale
                )
    ...
    def _ensure_gate_mask(self):
        ...
        if not (hasattr(self, "_terrain") and hasattr(self._terrain, "terrain_types")):
            self._gate_mask = torch.ones(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            self._discrete_terrain_mask = torch.zeros(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            self._flat_terrain_mask = torch.zeros(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            self._real_terrain_mask = torch.ones(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            self._boxes_terrain_mask = torch.zeros(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            self._stairs_down_terrain_mask = torch.zeros(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            self._stairs_up_terrain_mask = torch.zeros(
                self.num_envs, dtype=torch.bool, device=self.device
            )
            return
        sg = self.cfg.terrain.terrain_generator
        props = np.array(
            [s.proportion for s in sg.sub_terrains.values()], dtype=np.float64
        )
        props = props / props.sum()
        cum = np.cumsum(props)
        ncol = sg.num_cols
        ct = np.array(
            [int(np.min(np.where(c / ncol + 1e-3 < cum)[0])) for c in range(ncol)]
        )
        ...
        self._boxes_terrain_mask = (
            et == self._type_names.index("boxes")
            if "boxes" in self._type_names
            else torch.zeros_like(et, dtype=torch.bool)
        )
        self._stairs_down_terrain_mask = (
            et == self._type_names.index("stairs")
            if "stairs" in self._type_names
            else torch.zeros_like(et, dtype=torch.bool)
        )
        self._stairs_up_terrain_mask = (
            et == self._type_names.index("stairs_up")
            if "stairs_up" in self._type_names
            else torch.zeros_like(et, dtype=torch.bool)
        )
        print(
            f"[GATE] curriculum gates exclude hard terrain types {[n for n in ('slope_inv', 'boxes', 'stairs_up') if n in self._type_names]}"
            ...
            f" | discrete(stairs/boxes/stairs_up)={int(self._discrete_terrain_mask.sum())}/{self.num_envs}",
            flush=True,
        )
    ...
    def _restore_curriculum_from_telemetry(self, telemetry_path: Path) -> bool:
        ...
        if not restored and curriculum.get("terrain_real_mean") is not None:
            level = max(
                0, min(max_level, int(round(float(curriculum["terrain_real_mean"]))))
            )
    ...
    def _select_legacy_curriculum_telemetry(self, source_run: Path) -> Path:
        ...
        def _score(path: Path) -> float:
            ...
            try:
                ...
                return float(
                    curriculum.get(
                        "terrain_real_mean", curriculum.get("terrain_mean", 0.0)
                    )
                )
            except Exception:
                return -1.0
        ...
        return (
            max(candidates, key=_score)
            if candidates
            else source_run / "train.telemetry.jsonl"
        )
    ...
    def _restore_curriculum_state(self) -> None:
        ...
        try:
            if not state_path.is_file():
                self._restore_curriculum_from_telemetry(
                    self._select_legacy_curriculum_telemetry(source_run)
                )
                if int(getattr(self.cfg, "terrain_curriculum_state_schema", 1)) >= 2:
                    self._reset_stair_curriculum_levels()
                    print(
                        "[CURRICULUM] legacy stairs reset for event-complete schema v2",
                        flush=True,
                    )
                return
            ...
            current_schema = int(
                getattr(self.cfg, "terrain_curriculum_state_schema", 1)
            )
            # 全局课程状态与环境数量无关；即使逐环境地形布局需要按均值重建，也应保留。
            self._phase = max(int(self._phase), int(state.get("phase", self._phase)))
            self._dr_level = max(
                int(self._dr_level), int(state.get("dr_level", self._dr_level))
            )
            self._dr_gate_count = max(
                int(self._dr_gate_count), int(state.get("dr_gate_count", 0))
            )
            self._penalty_gate = max(
                float(self._penalty_gate), float(state.get("penalty_gate", 0.0))
            )
            self._clearance_gate = max(
                float(self._clearance_gate), float(state.get("clearance_gate", 0.0))
            )
            ...
            if not isinstance(levels, list) or len(levels) != self.num_envs:
                raise ValueError(
                    f"terrain_levels length mismatch: {len(levels or [])} != {self.num_envs}"
                )
            if types != current_types:
                raise ValueError("terrain type layout differs from the saved run")
            self._terrain.terrain_levels.copy_(
                torch.as_tensor(
                    levels, dtype=self._terrain.terrain_levels.dtype, device=self.device
                )
            )
            ...
            print(
                f"[CURRICULUM] exact restore failed ({type(exc).__name__}: {exc}); trying telemetry",
                flush=True,
            )
    ...
    def _save_curriculum_state(self, step: int) -> None:
        ...
        streak = (
            getattr(self, "_terrain_move_down_streak", torch.zeros_like(levels))
            .detach()
            .cpu()
        )
        state = {
            ...
            "terrain_boxes_success_ema": float(
                getattr(self, "_terrain_boxes_success_ema", 0.0)
            ),
            "terrain_stairs_down_success_ema": float(
                getattr(self, "_terrain_stairs_down_success_ema", 0.0)
            ),
            "terrain_stairs_up_success_ema": float(
                getattr(self, "_terrain_stairs_up_success_ema", 0.0)
            ),
            "terrain_boxes_collapse_ema": float(
                getattr(self, "_terrain_boxes_collapse_ema", 1.0)
            ),
            "terrain_stairs_down_collapse_ema": float(
                getattr(self, "_terrain_stairs_down_collapse_ema", 1.0)
            ),
            "terrain_stairs_up_collapse_ema": float(
                getattr(self, "_terrain_stairs_up_collapse_ema", 1.0)
            ),
        }
    ...
    def _terrain_level_stats(self) -> dict[str, float | int]:
        ...
        _fill(
            "terrain_flat",
            getattr(self, "_flat_terrain_mask", torch.zeros_like(lv, dtype=torch.bool)),
        )
        _fill(
            "terrain_real",
            getattr(self, "_real_terrain_mask", torch.ones_like(lv, dtype=torch.bool)),
        )
        _fill(
            "terrain_discrete",
            getattr(
                self, "_discrete_terrain_mask", torch.zeros_like(lv, dtype=torch.bool)
            ),
        )
    ...
    def _apply_action(self):
        ...
        self.robot.set_joint_position_target(
            self.action_offset + self.action_scale * self._delayed_action
        )
    ...
    def _accumulate_curriculum_progress(self):
        ...
        self._curriculum_forward_dist += torch.where(
            moving, step_progress, torch.zeros_like(step_progress)
        )
        self._curriculum_cmd_distance += cmd_mag * float(
            self.cfg.dt * self.cfg.decimation
        )
    ...
    def _compute_terrain_ctx(self):
        ...
        fy = s * dx + c * dy
        dz = torch.nan_to_num(
            hits[..., 2] - base[:, 2:3], nan=0.0, posinf=0.0, neginf=0.0
        )
    ...
    def _amp_terrain_context(self):
        ...
        response = torch.clamp(
            torch.as_tensor(
                response, dtype=self._terrain_ctx.dtype, device=self.device
            ),
            0.0,
            1.0,
        )
    ...
    def _compute_amp_obs(self):
        ...
        rel_b = self._feet_rel_base(
            self.robot.data.body_pos_w[:, self.foot_indexes],
    ...
    def _get_observations(self) -> dict:
        ...
        gait_obs = torch.cat(
            [torch.sin(2 * math.pi * lp), torch.cos(2 * math.pi * lp)], dim=-1
        )  # (N, 8)
        N, dev = self.num_envs, self.device
        # 观测噪声：缩放前加入，使量级接近真实传感器规格。
        jpos_n = (
            self.robot.data.joint_pos
            + torch.randn(N, 12, device=dev) * self.cfg.obs_noise_jpos
        )
        jvel_n = (
            self.robot.data.joint_vel
            + torch.randn(N, 12, device=dev) * self.cfg.obs_noise_jvel
        )
        angv_n = (
            self.robot.data.root_ang_vel_b
            + torch.randn(N, 3, device=dev) * self.cfg.obs_noise_angvel
        )
        grav_n = (
            self.robot.data.projected_gravity_b
            + torch.randn(N, 3, device=dev) * self.cfg.obs_noise_gravity
        )
        ...
        self._obs_history = torch.cat(
            [prop_now.unsqueeze(1), self._obs_history[:, :-1]], dim=1
        )  # FIFO
        # 足端接触缓存，供 _get_rewards 复用，避免重复查询。
        forces_now = self._contact_sensor.data.net_forces_w[
        ...
        hits = self._height_scanner.data.ray_hits_w  # (N, P, 3)
        hscan = (
            self.robot.data.root_pos_w[:, 2:3] - hits[:, :, 2] - self.cfg.stand_height
        ).clamp(-1.0, 1.0)
        ...
        if not bool(finite.all()):
            ...
            print(
                f"[NANGUARD] step={self._log_step} non-finite policy-obs in {int(bad.numel())} env(s): "
                f"{bad[:8].tolist()} -> sanitized",
                flush=True,
            )
        ...
        if getattr(self, "_amp_frame_stride", 1) > 1:
            ...
            for i in reversed(range(self.cfg.num_amp_observations - 1)):
                self.amp_observation_buffer[:, i + 1] = self.amp_observation_buffer[
                    :, i
                ]
        ...
        self._log_step += 1
        if self._log_step % self.cfg.log_every == 0 and not getattr(
    ...
    def _log_training_diag(self):
        ...
        with torch.no_grad():
            ...
            self._ensure_gate_mask()
            upright = (
                (self.robot.data.projected_gravity_b[self._gate_mask, 2] < -0.7)
                .float()
                .mean()
                .item()
            )
        ...
        stairs_down_collapse = float(
            getattr(self, "_terrain_stairs_down_collapse_ema", 1.0)
        )
        stairs_up_collapse = float(
            getattr(self, "_terrain_stairs_up_collapse_ema", 1.0)
        )
        ...
        _below = (
            self.robot.data.root_pos_w[self._gate_mask, 2] < self.cfg.termination_height
        )
        _tilted = (
            self.robot.data.projected_gravity_b[self._gate_mask, 2] >= -0.7
        )  # 非直立。
        ...
        min_prog, active_dirs = active_direction_progress(
            progress_by_dir, self.cfg, self._phase
        )
        ...
        self._sustained_execution_gate = (
            min(sustained_values) if sustained_values else 0.0
        )
        ...
        if terrain_phase:
            self._clearance_gate = min(
                1.0, self._clearance_gate + 1.0 / C.clearance_ramp_intervals
            )
        terrain_slip_thr = float(getattr(C, "terrain_gate_slip_high", 0.22))
        terrain_health_ok = (
            getattr(self, "_slip_high_fraction", 0.0) <= terrain_slip_thr
        )
        ...
        ) and (not terrain_phase or terrain_health_ok)
        ...
        flat_core_checks = {
            ...
            "false_terrain_response": float(
                flat_metrics.get("false_terrain_response", 1e9)
            )
            <= _phase_thr("flat_false_terrain_response", 0.01),
        }
        flat_gait_checks = {
            "diagonal": float(flat_metrics.get("diagonal_contact", 0.0)) >= diag_thr,
            "duty_target": float(flat_metrics.get("duty_target", 0.0))
            >= duty_target_thr,
            "duty_symmetry": float(flat_metrics.get("duty_symmetry", 0.0))
            >= duty_symmetry_thr,
        ...
        phase_spec = (
            phase_command_spec(C, self._phase) if phase_command_spec is not None else {}
        )
        command_mode = str(phase_spec.get("command_mode", ""))
        is_mixed_phase = command_mode == "mixed"
        terrain_gate_level = (
            real_tlvl
            if float(terrain_stats.get("terrain_real_frac", 0.0)) > 0.0
            else tlvl
        )
        ...
        terrain_capability_gate_ok = (
            ...
            and stairs_down_success
            >= float(getattr(C, "phase_gate_stairs_down_success_2", 0.0))
            and stairs_up_success
            >= float(getattr(C, "phase_gate_stairs_up_success_2", 0.0))
            and boxes_collapse <= float(getattr(C, "phase_gate_boxes_collapse_2", 1.0))
            and stairs_down_collapse
            <= float(getattr(C, "phase_gate_stairs_down_collapse_2", 1.0))
            and stairs_up_collapse
            <= float(getattr(C, "phase_gate_stairs_up_collapse_2", 1.0))
        )
        if terrain_phase:
            gate = (
                min_prog >= prog_thr
        ...
        dynamic_advance = {
            ...
            if result.get("scope") == "curriculum"
            and result.get("action") in {"advance", "hold"}
        }
        dynamic_curriculum_ok = all(
            bool(result.get("passed")) for result in dynamic_advance.values()
        )
        ...
        self._phase_gate_values = {
            ...
            "slip": self._slip_ema,
            "diagonal": float(flat_metrics.get("diagonal_contact", diag_contact))
            if terrain_phase
            else diag_contact,
            "duty_target": float(flat_metrics.get("duty_target", duty_target_score))
            if terrain_phase
            else duty_target_score,
            "duty_symmetry": float(
                flat_metrics.get("duty_symmetry", duty_symmetry_score)
            )
            if terrain_phase
            else duty_symmetry_score,
            "duty_valid": float(flat_metrics.get("duty_valid", duty_valid_frac))
            if terrain_phase
            else duty_valid_frac,
            "period": float(flat_metrics.get("period", period_score))
            if terrain_phase
            else period_score,
            ...
            "flat_false_terrain_response": float(
                flat_metrics.get("false_terrain_response", 1e9)
            ),
        ...
        if (self._penalty_gate >= 1.0 or not self._penalty_budget_controls_quality) and getattr(
            self, "_phase_penalty_full_step", None
        ) is None:
        ...
        if self._phase < getattr(self, "_max_training_phase", 3):
            self._phase_count = (
                self._phase_count + 1 if gate else max(0, self._phase_count - 1)
            )
            if self._phase_count >= C.phase_intervals or _timed_out:
                ...
                _tag = (
                    " FORCED(deadlock-safeguard: min_prog<prog_thr for phase_max_steps post-ramp)"
                    if _forced
                    else ""
                )
                print(
                    f"[PHASE] -> {self._phase} at step {self._log_step}{_tag} "
                    f"(prog={min_prog:.2f} gait={gm:.2f} slip={self._slip_ema:.2f} terrain={tlvl:.2f})",
                    flush=True,
                )
        ...
        if self._vel_curriculum_enable and self._advance_ok:
            if self._fwd_prog >= C.vel_cur_up:
                self._vel_max_fwd = min(self._vel_max_fwd + step, C.cmd_fwd_max)
            elif self._fwd_prog <= C.vel_cur_down:
                self._vel_max_fwd = max(self._vel_max_fwd - step, self._vel_floor_fwd)
            if self._back_prog >= C.vel_cur_up:
                self._vel_max_back = min(self._vel_max_back + step, C.cmd_back_max)
            elif self._back_prog <= C.vel_cur_down:
                self._vel_max_back = max(
                    self._vel_max_back - step, self._vel_floor_back
                )
            if self._lat_prog >= C.vel_cur_up:
                self._vel_max_lat = min(self._vel_max_lat + step, C.cmd_lat_max)
            elif self._lat_prog <= C.vel_cur_down:
                self._vel_max_lat = max(self._vel_max_lat - step, self._vel_floor_lat)
            # yaw 使用专用增长阈值，避免共享 vel_cur_up 对 yaw 过高导致只能缩小不能扩大。
            _yaw_up = float(getattr(C, "yaw_vel_cur_up", 0.40))
            if self._yaw_prog >= _yaw_up:
                self._vel_max_yaw = min(self._vel_max_yaw + step, C.cmd_yaw_max)
            elif self._yaw_prog <= C.vel_cur_down:
                self._vel_max_yaw = max(self._vel_max_yaw - step, self._vel_floor_yaw)
        ...
        self._dr_gate_progress_required = float(
            [C.dr_gate_progress, C.dr_gate_progress_l2, C.dr_gate_progress_l3][
                min(self._dr_level, 2)
            ]
        )
        ...
        if (
            ...
            gate_prog = [
                C.dr_gate_progress,
                C.dr_gate_progress_l2,
                C.dr_gate_progress_l3,
            ][self._dr_level]
            ...
            transition_final_thresholds = {
                ...
                "reverse": float(
                    getattr(C, "dr_gate_transition_reverse_fail_final", 0.30)
                ),
            ...
            handoff_fail_final = float(
                getattr(C, "dr_gate_transition_handoff_fail_final", 0.10)
            )
            ...
            if self._dr_gate_eligible:
                ...
                if self._dr_gate_count >= C.dr_gate_intervals:
                    self._dr_level += 1
                    self._dr_gate_count = 0
                    push_s = getattr(C, f"dr_push_interval_s_{self._dr_level}", 0.0)
                    self._push_steps = (
                        max(1, int(push_s / (C.dt * C.decimation)))
                        if push_s > 0
                        else int(1e9)
                    )
                    print(
                        f"[DR] Level up -> {self._dr_level} at step {self._log_step}",
                        flush=True,
                    )
            else:
        ...
        if hasattr(self, "_terrain") and hasattr(self._terrain, "terrain_types"):
            if not hasattr(self, "_col_type"):
                ...
                ct = np.array(
                    [
                        int(np.min(np.where(c / ncol + 1e-3 < cum)[0]))
                        for c in range(ncol)
                    ]
                )
                self._col_type = torch.as_tensor(ct, device=self.device)
            ...
            parts = [
                f"{nm[:5]}={float(lv[env_type == ti].mean()):.1f}"
                for ti, nm in enumerate(self._type_names)
                if bool((env_type == ti).any())
            ]
        ...
        print(
            f"[ENV s={self._log_step}] PHASE={self._phase} pen_gate={self._penalty_gate:.2f} "
            ...
            f"act_std={act_std:.3f} cmd_frac={fwd:.2f}/{bwd:.2f}/{lat:.2f}/{yaw:.2f}",
            flush=True,
        )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        cmd_lin_m2 = torch.sum(
            cmd_lin * cmd_lin, dim=1
        )  # ||cmd_lin||^2，供下方门控使用。
        ...
        lin_prog = torch.clamp(
            torch.sum(vel_lin * cmd_lin, dim=1) / cmd_lin_m2.clamp(min=thr), 0.0, 1.0
        )
        ang_prog = torch.clamp(wz * cmd_ang / cmd_ang_2.clamp(min=thr), 0.0, 1.0)
        lin_stand = torch.exp(
            -torch.sum(vel_lin * vel_lin, dim=1) / self.cfg.stand_sigma
        )
        ang_stand = torch.exp(-wz * wz / self.cfg.stand_sigma)
        lin_stand_w = torch.where(
            mv_a & ~mv_l, 0.25, 1.0
        )  # 纯 yaw 时不对非命令线速度轴给满额站立奖励。
        ang_stand_w = torch.where(mv_l & ~mv_a, 0.25, 1.0)
        r_lin = self.cfg.rew_track_lin * torch.where(
            mv_l, lin_prog, lin_stand * lin_stand_w
        )
        r_ang = self.cfg.rew_track_ang * torch.where(
            mv_a, ang_prog, ang_stand * ang_stand_w
        )
        r_alive = self.cfg.rew_alive * (~self.reset_terminated).float()
        r_arate = self.cfg.rew_action_rate * torch.sum(
            torch.square(self.actions - self.last_actions), dim=1
        )
        r_jacc = self.cfg.rew_joint_acc * torch.sum(
            torch.square(self.robot.data.joint_acc), dim=1
        )
        ...
        try:
            ...
            tau_over = torch.clamp(
                tau.abs() - self.cfg.torque_limit_frac * eff_lim, min=0.0
            )
            ...
            if "torque" not in self._dr_warned:
                print(
                    f"[REW] torque penalty unavailable ({type(_e).__name__}); skipping",
                    flush=True,
                )
                self._dr_warned.add("torque")
        r_vz = self.cfg.rew_lin_vel_z * torch.square(
            self.robot.data.root_lin_vel_b[:, 2]
        )
        r_wxy = self.cfg.rew_ang_vel_xy * torch.sum(
        ...
        vel_along = (vb2 * cmd_lin).sum(-1) / cmd_lin_mag.clamp(
            min=0.1
        )  # 沿命令方向速度。
        yaw_match = (
        ...
        air_credit = torch.clamp(
            air_time - self.cfg.air_time_min,
            min=0.0,
            max=self.cfg.air_time_target - self.cfg.air_time_min,
        )
        r_air = (
            self.cfg.rew_feet_air_time
            * torch.sum(air_credit * first_contact.float(), dim=1)
            * effective
        )
        mv_any = mv_l | mv_a
        ...
        desired_stance = (
            leg_phase < self.cfg.gait_duty
        ).float()  # 1 表示应处于支撑相。
        gait_match = (
            desired_stance * in_contact + (1.0 - desired_stance) * (1.0 - in_contact)
        ).mean(dim=1)
        # 原始 gait_match 不加 dead-zone，使 bootstrap 时也有接触节律梯度。
        # 粗糙地形上降低固定节律权重，允许策略打破平地小跑节律去跨越地形。
        if self.cfg.terrain_ctx_dim >= 3:
            rough_gate = torch.clamp(
                self._terrain_ctx[:, 2] / self.cfg.gait_rough_scale, 0.0, 1.0
            )
        else:
            rough_gate = torch.zeros(self.num_envs, device=self.device)
        r_gait = (
            self.cfg.rew_gait_phase * gait_match * mv_any.float() * (1.0 - rough_gate)
        )
        ...
        if self.cfg.rew_imitate != 0.0:
            ...
            T_im = torch.clamp(
                self.cfg.gait_period - self.cfg.gait_period_slope * spd_im,
                min=self.cfg.gait_period_min,
                max=self.cfg.gait_period,
            )
            rough_im = (
                self._terrain_ctx[:, 2] if self.cfg.terrain_ctx_dim >= 3 else None
            )
            jp_ref = flat_reference(
                self.commands,
                self._gait_phase * T_im,
                gait_period=self.cfg.gait_period,
                gait_period_slope=self.cfg.gait_period_slope,
                gait_period_min=self.cfg.gait_period_min,
            ...
            self._style_err = (
                float(jdiff.abs().mean(dim=1)[mv_any].mean())
                if bool(mv_any.any())
        ...
        r_swing_drag = (
            self.cfg.rew_swing_drag
            * ((1.0 - desired_stance) * in_contact).mean(dim=1)
            * mv_any.float()
            * rough_gate
        )
        ...
        off_sched = (1.0 - desired_stance) * in_contact + desired_stance * (
            1.0 - in_contact
        )  # (N,4)
        _ge_relax = (1.0 - rough_gate) if (self.cfg.terrain_ctx_dim >= 3) else 1.0
        r_gait_enforce = (
            self.cfg.rew_gait_enforce
            * off_sched.mean(dim=1)
            * mv_any.float()
            * _ge_relax
        )
        ...
        r_offaxis = (
            self.cfg.rew_offaxis_vel * torch.sum(v_perp * v_perp, dim=1) * mv_l.float()
        )
        ...
        r_overshoot = self.cfg.rew_overshoot * (
            lin_over * lin_over * mv_l.float() + ang_over * ang_over * mv_a.float()
        )
        ...
        ang_under = torch.clamp(
            (cmd_ang.abs() - ang_tol) - wz * torch.sign(cmd_ang), min=0.0
        )
        r_underspeed = self.cfg.rew_underspeed * (
            lin_under * lin_under * mv_l.float() + ang_under * ang_under * mv_a.float()
        )
        back_cmd = (
        ...
        back_speed_deficit = torch.clamp(
            (self.commands[:, 0].abs() - lin_tol) - (-vel_lin[:, 0]), min=0.0
        )
        ...
        lat_speed_deficit = torch.clamp(
            (self.commands[:, 1].abs() - lin_tol) - lat_signed_speed, min=0.0
        )
        r_lateral_aux = (
            self.cfg.rew_lateral_underspeed
            * lat_speed_deficit
            * lat_speed_deficit
            * lat_cmd.float()
        )
        ...
        r_wrong_dir = self.cfg.rew_wrong_dir * (
            lin_wrong * lin_wrong * mv_l.float() + ang_wrong * ang_wrong * mv_a.float()
        )
        ...
        r_height = self.cfg.rew_base_height * torch.clamp(
            self.cfg.stand_height - hgt, min=0.0
        )

        # 站立项由直立、默认关节、高度和四足接触四部分组成。
        standing = (~mv_l) & (~mv_a)
        up = torch.clamp(
            -self.robot.data.projected_gravity_b[:, 2], 0.0, 1.0
        )  # 1 表示直立。
        # jdev 使用较宽 Gaussian，保证趴下时仍有回到默认姿态的梯度。
        jdev = torch.exp(
            -torch.sum((self.robot.data.joint_pos - self.action_offset) ** 2, dim=1)
            / 4.0
        )
        ...
        r_stand = (
            self.cfg.rew_stand_pose
            * ((up + jdev + hdev + feet_down) / 4.0)
            * standing.float()
        )
        # 站立静止：零命令时惩罚关节速度，使机器人平滑回到立正而不是碎步抖动。
        r_stand_still = (
            self.cfg.rew_stand_still
            * torch.sum(self.robot.data.joint_vel**2, dim=1)
            * standing.float()
        )

        # 髋关节中立：非横移命令下压制髋关节外翻/内扣；横移时关闭该约束。
        lat_active = torch.clamp(
            self.commands[:, 1].abs() / self.cfg.hip_neutral_lat_scale, 0.0, 1.0
        )
        ...
        q = (
            self.robot.data.root_quat_w[:, None, :]
        ...
        s_swing = ((leg_phase - self.cfg.gait_duty) / (1.0 - self.cfg.gait_duty)).clamp(
            0.0, 1.0
        )  # (N,4)
        lift_w = torch.clamp(
            (0.65 - s_swing) / 0.20, 0.0, 1.0
        )  # 1 in lift/apex, 0 by s_swing=0.65
        land_w = torch.clamp(
            (s_swing - 0.70) / 0.30, 0.0, 1.0
        )  # 0 until touchdown phase, 1 at contact

        # 1. 支撑滑移：惩罚接触脚的水平滑动。
        slip_speed = foot_vel_b_xy.norm(dim=-1)  # (N,4), m/s
        r_stance_slip = (
            self.cfg.rew_stance_slip
            * (in_contact * slip_speed).sum(dim=1)
            * mv_any.float()
        )
        # 平均支撑滑移暴露给阶段门控。
        slip_now_env = (in_contact * slip_speed).sum(dim=1) / in_contact.sum(
            dim=1
        ).clamp(min=1.0)
        self._slip_now = float(slip_now_env.mean())

        # 2. 摆动方向：摆动脚应沿命令方向移动；纯 yaw 使用绕 base 旋转产生的切向方向。
        foot_rel_b = self._feet_rel_base(
            self.robot.data.body_pos_w[:, self.foot_indexes],
            self.robot.data.root_pos_w,
            self.robot.data.root_quat_w,
            self.n_feet,
        ...
        cmd_hat_2d = cmd_lin / cmd_lin_mag[:, None].clamp(
            min=1e-4
        )  # (N,2) unit command dir
        yaw_vec = torch.stack(
            [
                -cmd_ang[:, None] * foot_rel_b[:, :, 1],
                cmd_ang[:, None] * foot_rel_b[:, :, 0],
            ],
            dim=-1,
        )
        ...
        r_swing_dir = self.cfg.rew_swing_dir * (
            vel_along_cmd * swing_mask * lift_gate
        ).mean(dim=1)

        # 2b. 落脚减速：落脚阶段惩罚足端水平速度，使足端在触地前刹住。
        r_land_decel = self.cfg.rew_land_decel * (
            slip_speed * swing_mask * land_w
        ).mean(dim=1)

        # 3. clearance：摆动脚应相对脚下/附近地形抬起，而不只是相对机身抬起。
        hits = (
            self._height_scanner.data.ray_hits_w
        )  # (N,P,3), yaw-aligned grid around base
        finite_hits = torch.isfinite(hits[:, :, 2])
        hits_z_valid = torch.where(
            finite_hits, hits[:, :, 2], torch.zeros_like(hits[:, :, 2])
        )
        ...
        support_z_contact = (ground_z * contact_w).sum(dim=1) / contact_sum.clamp(
            min=1.0
        )
        ...
        cmdw_ahead = _command_xy_world_from_root_yaw(
            self.robot.data.root_quat_w, self.commands[:, :2]
        )
        ...
        ahead_max = (
            torch.where(ahead_mask, hz_hi, torch.full_like(hz_hi, -1e3))
            .max(dim=1)
            .values
        )
        ahead_min = (
            torch.where(ahead_mask, hz_lo, torch.full_like(hz_lo, 1e3))
            .min(dim=1)
            .values
        )
        ahead_count = ahead_mask.float().sum(dim=1)
        ahead_s_min = (
            torch.where(ahead_mask, ahead_s, torch.full_like(ahead_s, 1e3))
            .min(dim=1)
            .values
        )
        ahead_s_max = (
        ...
        terrain_scan_probe = {
            ...
            "hits_z_span_mean": (
                hits_z_valid.max(dim=1).values - hits_z_valid.min(dim=1).values
            ).mean(),
            "rel_x_min": rel_x.min(),
            "rel_x_max": rel_x.max(),
            "rel_y_min": rel_y.min(),
            ...
            "ahead_s_min": torch.where(
                has_ahead, ahead_s_min, torch.zeros_like(ahead_s_min)
            ).mean(),
            "ahead_s_max": torch.where(
                has_ahead, ahead_s_max, torch.zeros_like(ahead_s_max)
            ).mean(),
            "support_z_mean": support_z.mean(),
            "support_z_min": support_z.min(),
            "support_z_max": support_z.max(),
            "ahead_max_z_mean": torch.where(
                has_ahead, ahead_max, torch.zeros_like(ahead_max)
            ).mean(),
            "ahead_min_z_mean": torch.where(
                has_ahead, ahead_min, torch.zeros_like(ahead_min)
            ).mean(),
            "raw_rise_mean": torch.where(
                has_ahead, ahead_max - support_z, torch.zeros_like(support_z)
            ).mean(),
            "raw_rise_max": torch.where(
                has_ahead, ahead_max - support_z, torch.zeros_like(support_z)
            ).max(),
            "raw_drop_mean": torch.where(
                has_ahead, support_z - ahead_min, torch.zeros_like(support_z)
            ).mean(),
            "raw_drop_max": torch.where(
                has_ahead, support_z - ahead_min, torch.zeros_like(support_z)
            ).max(),
        }
        terrain_rise_ahead = torch.where(
            has_ahead,
            torch.clamp(ahead_max - support_z, min=0.0),
            torch.zeros_like(support_z),
        )
        terrain_drop_ahead = torch.where(
            has_ahead,
            torch.clamp(support_z - ahead_min, min=0.0),
            torch.zeros_like(support_z),
        )
        # 粗糙度门控 clearance：只在真实粗糙/离散地形上加重抬脚，平地保持低而高效。
        # cg = 阶段渐入门控 x 每个 env 的粗糙度因子。
        rough = self._terrain_ctx[:, 2]  # (N,) terrain height std (m)
        rough_factor = torch.clamp(
            (rough - self.cfg.clr_rough_flat) / self.cfg.clr_rough_span, 0.0, 1.0
        )  # (N,)
        # 离散障碍：楼梯/boxes 可强制 rough_factor=1，避免低粗糙度读数导致抬脚目标不足。
        if getattr(self.cfg, "discrete_clearance", False):
            self._ensure_gate_mask()
            rough_factor = torch.where(
                self._discrete_terrain_mask, torch.ones_like(rough_factor), rough_factor
            )
        cg = self._clearance_gate * rough_factor  # (N,) 0 on flat, ->1 on rough
        # 权重：平地轻约束，粗糙地形逐步加重。
        eff_clearance_w = (
            self.cfg.rew_clearance
            + (self.cfg.rew_clearance_heavy - self.cfg.rew_clearance) * cg
        )  # (N,)
        # 目标：基础抬脚高度加粗糙地形 bonus。
        clearance_target = (
            self.cfg.base_clearance + self.cfg.clr_rough_bonus_max * cg[:, None]
        )  # (N,1)
        clr_deficit = torch.clamp(clearance_target - foot_clearance, min=0.0)
        # lift_gate：早期完整摆动窗口，后期渐入 lift/apex-only，释放自然落脚下降段。
        r_clearance = eff_clearance_w * (swing_mask * lift_gate * clr_deficit).mean(
            dim=1
        )
        ...
        if (
            w_climb > 0.0 or w_terrain_up > 0.0 or w_terrain_down > 0.0
        ) and self._phase >= getattr(self, "_terrain_start_phase", 5):
            self._ensure_gate_mask()
            disc = getattr(
                self,
            ...
            upright = (-self.robot.data.projected_gravity_b[:, 2]).clamp(
                0.0, 1.0
            ) > 0.85
            # 软滑移权重：目标以下保持满额，超过后逐步衰减，避免硬截断丢梯度。
            slip_target = float(getattr(self.cfg, "climb_slip_gate", 0.30))
            slip_span = max(
                float(getattr(self.cfg, "climb_slip_soft_span", 0.18)), 1e-6
            )
            slip_weight = torch.clamp(
                1.0 - torch.clamp(slip_now_env - slip_target, min=0.0) / slip_span,
                0.0,
                1.0,
            )
            common = (
                disc.float() * moving_lin_cmd.float() * upright.float() * slip_weight
            )
            ...
            up_cap = max(
                float(
                    getattr(
                        self.cfg,
                        "terrain_up_vz_cap",
                        getattr(self.cfg, "climb_vz_cap", 0.6),
                    )
                ),
                1e-6,
            )
            ...
            down_v = (-self.robot.data.root_lin_vel_w[:, 2]).clamp(
                min=0.0, max=down_cap
            )
            down_target = max(
                float(getattr(self.cfg, "terrain_down_vz_target", 0.18)), 1e-6
            )
            down_control = torch.clamp(
                1.0 - torch.clamp(down_v - down_target, min=0.0) / down_target, 0.0, 1.0
            )
        ...
        self._rew_dbg = {
            "lin": float(r_lin.mean()),
            "ang": float(r_ang.mean()),
            "gait": float(r_gait.mean()),
            "imit": float(r_imitate.mean()),
            "stand": float(r_stand.mean()),
            "height": float(r_height.mean()),
            "slip": float(r_stance_slip.mean()),
            "clear": float(r_clearance.mean()),
            "hip": float(r_hip.mean()),
            "offax": float(r_offaxis.mean()),
            "over": float(r_overshoot.mean()),
            "under": float(r_underspeed.mean()),
            "back_aux": float(r_backward_aux.mean()),
            "lat_aux": float(r_lateral_aux.mean()),
            "wrong": float(r_wrong_dir.mean()),
            "climb": float(r_climb.mean()),
            "terr_up": float(r_terrain_up.mean()),
            "terr_down": float(r_terrain_down.mean()),
            ...
            "land": float((pg * r_land_decel).mean()),
            "torq": float(r_torque.mean()),
            "arate": float(r_arate.mean()),
            "vz": float(r_vz.mean()),
            "wxy": float(r_wxy.mean()),
            "stand_still": float(
                r_stand_still.mean()
            ),  # 常开站立静止项，记录未额外 gate 的值。
        }
    ...
    def _get_dones(self):
        ...
        if self.cfg.early_termination:
            ...
            if hasattr(self, "_terrain_height_under_base"):
                base_h_local = (
                    self.robot.data.root_pos_w[:, 2] - self._terrain_height_under_base()
                )
            else:
        ...
        nonfinite = ~(
            torch.isfinite(self.robot.data.root_pos_w).all(dim=1)
    ...
    def _apply_actuator_gain_dr(self, env_ids, level: int):
        ...
        apply_prob = min(
            max(float(getattr(self.cfg, f"dr_apply_prob_{level}", 1.0)), 0.0), 1.0
        )
        ...
        try:
            ...
            if bool(apply_mask.any()):
                ...
                k_scale[apply_mask] = torch.empty(
                    selected, 1, device=self.device
                ).uniform_(*k_range)
                d_scale[apply_mask] = torch.empty(
                    selected, 1, device=self.device
                ).uniform_(*d_range)
            ...
            if "gain" not in self._dr_warned:
                print(
                    "[DR] gain API unavailable; skipping stiffness/damping DR",
                    flush=True,
                )
    ...
    def _reset_idx(self, env_ids):
        ...
        if self.cfg.terrain.terrain_type == "generator":
            ...
            elapsed_s = self.episode_length_buf[env_ids].float() * float(
                self.cfg.dt * self.cfg.decimation
            )
            ...
            current_cmd_dir = current_cmd_xy / torch.linalg.norm(
                current_cmd_xy, dim=1, keepdim=True
            ).clamp(min=1e-6)
            if hasattr(self, "_loaded_support_z"):
                ...
                current_support_z = root_z - float(
                    getattr(
                        self.cfg, "stand_height", taili_geometry.NOMINAL_BASE_HEIGHT
                    )
                )
            ...
            support_height_delta = (
                current_support_z - self._episode_start_support_z[env_ids]
            )
            ...
            terrain_curriculum_active = self._phase >= getattr(
                self, "_terrain_start_phase", 5
            )
            ...
            upright_score = (-self.robot.data.projected_gravity_b[env_ids, 2]).clamp(
                -1.0, 1.0
            )
            ...
            body_wxy = torch.linalg.norm(
                self.robot.data.root_ang_vel_b[env_ids, :2], dim=1
            )
            v_along = torch.sum(
                self.robot.data.root_lin_vel_b[env_ids, :2] * current_cmd_dir, dim=1
            )
            self._ensure_gate_mask()
            ...
            if hasattr(self, "_col_type") and hasattr(self, "_type_names"):
                ...
                if "stairs" in self._type_names:
                    ...
                    expected_height_direction = torch.where(
                        down_mask,
                        -torch.ones_like(expected_height_direction),
                        expected_height_direction,
                    )
                if "stairs_up" in self._type_names:
            ...
            if (
                not hasattr(self, "_terrain_level_peak")
                or self._terrain_level_peak.shape != self._terrain.terrain_levels.shape
            ):
                self._terrain_level_peak = self._terrain.terrain_levels.clone()
            if (
                not hasattr(self, "_terrain_move_down_streak")
                or self._terrain_move_down_streak.shape
                != self._terrain.terrain_levels.shape
            ):
                self._terrain_move_down_streak = torch.zeros_like(
                    self._terrain.terrain_levels
                )
            if terrain_unlocked:
                peak_next = torch.maximum(
            ...
            moves = compute_terrain_curriculum_moves(
                ...
                stair_height_min=float(
                    getattr(self.cfg, "terrain_curriculum_stair_height_min", 0.08)
                ),
                stair_forward_min=float(
                    getattr(self.cfg, "terrain_curriculum_stair_forward_min", 0.75)
                ),
                stable_h=float(getattr(self.cfg, "terrain_curriculum_stable_h", 0.42)),
                stable_upright=float(
                    getattr(self.cfg, "terrain_curriculum_stable_upright", 0.85)
                ),
                stable_contact_min=float(
                    getattr(self.cfg, "terrain_curriculum_stable_contact_min", 2.0)
                ),
                stable_wxy_max=float(
                    getattr(self.cfg, "terrain_curriculum_stable_wxy_max", 1.50)
                ),
                speed_cap_ratio=float(
                    getattr(self.cfg, "terrain_curriculum_speed_cap_ratio", 1.60)
                ),
                speed_cap_min=float(
                    getattr(self.cfg, "terrain_curriculum_speed_cap_min", 0.75)
                ),
                failure_h=float(
                    getattr(self.cfg, "terrain_curriculum_failure_h", 0.42)
                ),
                failure_upright=float(
                    getattr(self.cfg, "terrain_curriculum_failure_upright", 0.75)
                ),
                failure_wxy=float(
                    getattr(self.cfg, "terrain_curriculum_failure_wxy", 2.00)
                ),
            ...
            patience = max(
                1, int(getattr(self.cfg, "terrain_curriculum_move_down_patience", 1))
            )
            if patience > 1:
                streak = self._terrain_move_down_streak[env_ids]
                streak = torch.where(
                    move_down_low, streak + 1, torch.zeros_like(streak)
                )
            ...
            floor_after_peak = int(
                getattr(self.cfg, "terrain_curriculum_floor_after_peak", 0)
            )
            if floor_after_peak > 0 and terrain_curriculum_active:
                level_now = self._terrain.terrain_levels[env_ids]
                peak_drop = max(
                    0, int(getattr(self.cfg, "terrain_curriculum_peak_drop", 0))
                )
                peak_floor = torch.clamp(
                    self._terrain_level_peak[env_ids] - peak_drop, min=floor_after_peak
                )
            ...
            with torch.no_grad():
                ...
                self._terrain_curriculum_eligible_frac = float(
                    effective_eligible.float().mean()
                )
                self._terrain_curriculum_move_up_rate = float(move_up.float().sum() / n)
                self._terrain_curriculum_move_down_rate = float(
                    move_down.float().sum() / n
                )
                self._terrain_curriculum_failure_down_rate = float(
                    failure_down.float().sum() / n
                )
                self._terrain_curriculum_stable_end_rate = float(
                    (moves["stable_end"] & eligible_valid).float().sum() / n
                )
                self._terrain_curriculum_speed_ok_rate = float(
                    (moves["speed_controlled"] & eligible_valid).float().sum() / n
                )
                ...
                family_beta = min(
                    max(
                        float(
                            getattr(
                                self.cfg, "terrain_curriculum_success_ema_beta", 0.90
                            )
                        ),
                        0.0,
                    ),
                ...
                for family_name, family_mask in (
                    ...
                    success_sample = float(
                        (move_up & family_valid).float().sum() / family_count
                    )
                    collapse_sample = float(
                        (failure_down & family_valid).float().sum() / family_count
                    )
                    ...
                    if not hasattr(self, success_attr):
                        ...
                        setattr(
                            self,
                            success_attr,
                            family_beta * float(getattr(self, success_attr))
                            + (1.0 - family_beta) * success_sample,
                        )
                        setattr(
                            self,
                            collapse_attr,
                            family_beta * float(getattr(self, collapse_attr))
                            + (1.0 - family_beta) * collapse_sample,
                        )
                discrete_mask = (
                    getattr(
                        self,
                ...
                self._terrain_curriculum_discrete_move_up_rate = float(
                    (move_up & discrete_mask).float().sum() / dn
                )
                self._terrain_curriculum_discrete_move_down_rate = float(
                    (move_down & discrete_mask).float().sum() / dn
                )
                self._terrain_curriculum_discrete_failure_down_rate = float(
                    (failure_down & discrete_mask).float().sum() / dn
                )
                self._terrain_curriculum_discrete_stable_end_rate = float(
                    (moves["stable_end"] & discrete_mask).float().sum() / dn
                )
        ...
        self._cmd_age_s[env_ids] = float(
            getattr(self.cfg, "cmd_transition_max_s", 0.35)
        )
        ...
        if (
            ...
            and lvl == 0
            and self._phase
            >= getattr(self, "_dr_start_phase", self._terrain_start_phase)
        ):
        ...
        if self.cfg.dr_enable and lvl >= 1:
            ...
            try:
                ...
                masses = (
                    self.robot.root_physx_view.get_masses()
                )  # (N, n_bodies)，CPU tensor。
                if self._default_masses is None:
                    self._default_masses = masses[
                        :, 0
                    ].clone()  # 默认 root 质量，只缓存一次。
                eids = env_ids.detach().cpu()
                delta = torch.empty(n).uniform_(
                    *mass_range
                )  # CPU，与质量 tensor 同设备。
                delta[~apply_mask.detach().cpu()] = 0.0
                masses[eids, 0] = (
                    self._default_masses[eids] + delta
                )  # 恢复默认值后再加扰动。
                self.robot.root_physx_view.set_masses(masses, eids)
            except Exception as e:
                if "mass" not in self._dr_warned:
                    print(f"[DR] mass DR FAILED: {type(e).__name__}: {e}", flush=True)
                    self._dr_warned.add("mass")
            ...
            if lvl >= _dr_full_lvl:
                ...
                if lvl > self._dr_mat_level:
                    ...
                    fr_range = getattr(
                        self.cfg,
                        f"dr_friction_range_{lvl}",
                        self.cfg.dr_friction_range_3,
                    )
                    com_off = float(
                        getattr(
                            self.cfg, f"dr_com_offset_{lvl}", self.cfg.dr_com_offset_3
                        )
                    )
                    try:
                        mats = (
                            self.robot.root_physx_view.get_material_properties().clone()
                        )  # (N, n_shapes, 3)
                        fr = torch.zeros(Nenv, 1, 1, device=mats.device).uniform_(
                            *fr_range
                        )  # 绝对写入，避免漂移。
                        ...
                        if "fric" not in self._dr_warned:
                            print(
                                "[DR] friction API unavailable; skipping friction DR",
                                flush=True,
                            )
                            self._dr_warned.add("fric")
                    try:
                        coms = (
                            self.robot.root_physx_view.get_coms().clone()
                        )  # (N, n_bodies, ...)
                        if self._default_coms is None:
                            self._default_coms = coms.clone()  # 默认 CoM 只缓存一次。
                        coms = (
                            self._default_coms.clone()
                        )  # 每次写入前恢复默认，避免漂移。
                        off = torch.zeros(Nenv, 2, device=coms.device).uniform_(
                            -com_off, com_off
                        )
                        coms[:, 0, 0:2] += off
                        self.robot.root_physx_view.set_coms(
                            coms, torch.arange(Nenv, device=coms.device)
                        )
                    except Exception:
                        if "com" not in self._dr_warned:
                            print(
                                "[DR] CoM API unavailable; skipping CoM DR", flush=True
                            )
                            self._dr_warned.add("com")
                    self._dr_mat_level = lvl
                    print(
                        f"[DR] level-{lvl} friction {list(fr_range)} + CoM ±{com_off} randomized (all envs)",
                        flush=True,
                    )
                # IMU 偏置按 episode 重采样，属于观测层扰动。
                gb = float(
                    getattr(
                        self.cfg, f"dr_imu_gyro_bias_{lvl}", self.cfg.dr_imu_gyro_bias_3
                    )
                )
                vb = float(
                    getattr(
                        self.cfg, f"dr_imu_grav_bias_{lvl}", self.cfg.dr_imu_grav_bias_3
                    )
                )
                self._imu_bias[env_ids, 0:3] = torch.zeros(
                    n, 3, device=self.device
                ).uniform_(-gb, gb)
                self._imu_bias[env_ids, 3:6] = torch.zeros(
                    n, 3, device=self.device
                ).uniform_(-vb, vb)
        num = len(env_ids)
        ...
        tctx_arg = (
            self._amp_terrain_context()[env_ids]
            if self.cfg.terrain_ctx_dim > 0
            else None
        )
        ...
        times = (
            np.zeros(num) if start else self._motion_loader.sample_times(num, clip_ids)
        )
        ...
        root[:, 0:3] = (
            bp[:, self.motion_ref_body_index] + self._terrain.env_origins[env_ids]
        )
        root[:, 2] += 0.03
        ...
        stand_reset_mask = taili_amp_reference.apply_flat_stand_reset(
            ...
            sole_clearance=float(
                getattr(self.cfg, "flat_stand_reset_clearance", 0.003)
            ),
        )
    ...
    def collect_reference_motions(self, num_samples, current_times=None):
        ...
        times = (
            np.expand_dims(current_times, -1) - (1.0 / 50.0) * _stride * np.arange(K)
        ).flatten()  # 按 stride 取参考帧。
        ...
        if self.cfg.terrain_ctx_dim >= 3:
            rough_rep = amp_terrain_ctx[env_sel, 2].repeat_interleave(
                K, dim=0
            )  # (num_samples*K,)
        jp_clip, jv_clip, bh, tn, foot_rel = flat_reference(
            ...
            clearance_rough_gain=self.cfg.ref_clearance_rough_gain,
            stance_dx=self.cfg.stance_dx,
        )  # wider fore-aft stance (blind config 0.05); base 0.0
