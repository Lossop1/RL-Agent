...
parser.add_argument(
    "--push", type=float, default=1.0
)  # lateral push m/s (spec E1/E2 ≥1.0)
parser.add_argument(
    "--report",
    type=str,
    default="",
    help="Write E1/E2 verdicts as JSON (for physeval_suite).",
)
...
def main():
    ...
    ac["agent"].setdefault("experiment", {}).update(
    ...
    robot = base.robot
    elim = robot.actuators[
        "legs"
    ].effort_limit  # SAME source as env reward / physeval_blind F2

    def step(cmd):
        with torch.inference_mode():
            getattr(base, "_cmd_target", base.commands)[:] = cmd.unsqueeze(0).expand(
                base.num_envs, -1
            )
    ...
    def run_push_case(cmd, label, target_speed):
        ...
        recov_step = torch.full(
            (base.num_envs,), -1, dtype=torch.long, device=base.device
        )
        sat_hits = 0
        sat_tot = 0  # torque-saturation event counting
        ...
        rec_t = (
            float(recov_step[recovered].float().mean()) * DT
        ...
        print(
            f"{label}: fall_rate={fall_rate:.3f} recovery_time={rec_t:.2f}s "
            f"recovered_frac={float(recovered.float().mean()):.2f} sat_rate={sat_rate:.4f}",
            flush=True,
        )
    ...
    results = {}
    fr1, rt1, tr1, sat1 = run_push_case(
        torch.tensor([0.5, 0, 0], device=base.device), "E1 walking push", 0.5
    )
    results.update(ACC.score_E1(fr1, rt1, tr1, torque_saturated=sat1))
    fr2, rt2, ret2, _sat2 = run_push_case(
        torch.tensor([0.0, 0, 0], device=base.device), "E2 standing push", 0.0
    )
