...
def set_mirrors(hscan_perm, device):
    ...
    _self_check(op, os_, "obs")
    _self_check(ap, as_, "action")
    ...
    _ENABLED = True
    print(
        "[SYM] mirror augmentation ON (obs 670, action 12; self-check passed)",
        flush=True,
    )
...
def patch_memory_sample_all():
    ...
    def sample_all(self, names, mini_batches=1, sequence_length=1):
        batches = orig(
            self, names, mini_batches=mini_batches, sequence_length=sequence_length
        )
        if (
            not _ENABLED
            or not _ACTIVE
            or "states" not in names
            or "actions" not in names
        ):
            return batches
        si = names.index("states")
        ai = names.index("actions")
        out = []
        for mb in batches:
            mb = list(mb)
