...
FOOT_OFF = np.array(
    [0.0252765, 0.0, -0.3179635]
)  # calf->foot (URDF FL_foot_joint xyz, exact)
...
def foot_traj_rel_hip(cfg, lg, t):
    ...
    for i, pp in enumerate(p):
        if pp < 0.5:  # stance: planted, moving back rel body
            ...
            h = (
                -4.0 * s**3 + 6.0 * s**2 - s
            )  # velocity-matched retraction: h'(0)=h'(1)=-1 -> no touchdown scuff
            fx[i] = x0 - sdx / 2 + sdx * h
            fy[i] = -sdy / 2 + sdy * h
            fz[i] = (
                -H0 + cfg.clearance * np.sin(np.pi * s) ** 2
            )  # sin^2 -> dz/dt=0 at touchdown (no slam)
    return fx, fy, fz
...
def solve_leg(cfg, lg, t):
    ...
    for i in range(N):
        ...
        seed = (a[0], a[1], a[2])
        f = fk_foot_3dof(*a)
        maxerr = max(maxerr, np.linalg.norm(f - np.array([fx[i], fy[i], fz[i]])))
...
def generate(cfg: GaitCfg, out_dir: str):
    ...
    body_names = ["base_link"] + [
        f"{lg}_{seg}" for lg in LEGS for seg in ["hip", "thigh", "calf", "foot"]
    ]
    B = len(body_names)
    bpos = np.zeros((N, B, 3))
    brot = np.zeros((N, B, 4))
    ...
    for i in range(N):
        ...
        Tb = H(Rb, pos[i])
        bpos[i, 0] = Tb[:3, 3]
        brot[i, 0] = rot2quat(Tb[:3, :3])
        bi = 1
        for lg in LEGS:
            sx, sy = SGN[lg]
            Th = (
                Tb
                @ trans([sx * HIPX, sy * HIPY, -0.0003])
                @ H(Rx(hip_a[lg][i]), [0, 0, 0])
            )
            ...
            for Tm in [Th, Tt, Tc, Tf]:
                bpos[i, bi] = Tm[:3, 3]
                brot[i, bi] = rot2quat(Tm[:3, :3])
                bi += 1
    blin = np.gradient(bpos, 1.0 / cfg.fps, axis=0)
    bang = np.zeros((N, B, 3))
    bang[:, 0] = quat_angular_velocity(
        brot[:, 0], cfg.fps
    )  # base only (legs approx 0 for AMP)

    os.makedirs(out_dir, exist_ok=True)
    out = os.path.join(out_dir, f"taili_{cfg.name}.npz")
    np.savez(
        out,
        fps=np.int64(cfg.fps),
        dof_names=np.array(dof_names),
        body_names=np.array(body_names),
        dof_positions=dof_pos.astype(np.float32),
        dof_velocities=dof_vel.astype(np.float32),
        body_positions=bpos.astype(np.float32),
        body_rotations=brot.astype(np.float32),
        body_linear_velocities=blin.astype(np.float32),
        body_angular_velocities=bang.astype(np.float32),
        ...
        cond_command=np.array(
            [cfg.vx, cfg.vy, cfg.wz], dtype=np.float32
        ),  # commanded vel
        cond_slope=np.array(
            [cfg.slope_x, cfg.slope_y], dtype=np.float32
        ),  # world-frame slope grad
    ...
    def quat_inv_rot(
        q, v
    ):  # rotate world vector v by inverse of quat q (wxyz) -> body frame
        w, x, y, z = q
        R = np.array(
            [
    ...
    diag_flrr = np.corrcoef(fl, rr)[0, 1]
    diag_flfr = np.corrcoef(fl, fr)[0, 1]
    lift = (fw.max() - fw.min()) * 100
    maxerr = max(errs.values())
    print(
        f"[{cfg.name}] frames={N} dur={N / cfg.fps:.1f}s ik_maxerr={maxerr:.2e} "
        f"terrain={cfg.terrain_type}"
    )
    print(
        f"   base: x {bpos[0, 0, 0]:+.2f}->{bpos[-1, 0, 0]:+.2f}  y {bpos[0, 0, 1]:+.2f}->{bpos[-1, 0, 1]:+.2f}  "
        f"yaw {yaw[-1]:+.2f}rad  z {bpos[0, 0, 2]:.2f}->{bpos[-1, 0, 2]:.2f}"
    )
    print(
        f"   foot z (body frame): swing={lift:.1f}cm  diag FL-RR={diag_flrr:+.2f} FL-FR={diag_flfr:+.2f}"
    )
...
def gait_library():
    return {
        ...
        "uphill_fwd": GaitCfg(
            "uphill_fwd", vx=+0.35, slope_x=+SLOPE, terrain_type="slope"
        ),
        "downhill_fwd": GaitCfg(
            "downhill_fwd", vx=+0.35, slope_x=-SLOPE, terrain_type="slope"
        ),
        "uphill_yawl": GaitCfg(
            "uphill_yawl", vx=+0.25, wz=+0.4, slope_x=+SLOPE, terrain_type="slope"
        ),
        "uphill_yawr": GaitCfg(
            "uphill_yawr", vx=+0.25, wz=-0.4, slope_x=+SLOPE, terrain_type="slope"
        ),
        # --- cross-slope (asymmetric load, not discrete foothold; fwd/back) ---
        "cross_fwd": GaitCfg(
            "cross_fwd", vx=+0.35, slope_y=+SLOPE, terrain_type="slope"
        ),
        "cross_back": GaitCfg(
            "cross_back", vx=-0.35, slope_y=+SLOPE, terrain_type="slope"
        ),
    }
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--gait", default="layerA", help="name | flat6 | flatms | slope6 | layerA | all"
    )
