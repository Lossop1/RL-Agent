...
### P4.4 超参数搜索空间
- **问题**：定义可搜索的超参数及其范围
- **状态**：进行中（拆分为 6 个原子任务，仅第 1 个完成）
- **阻塞**：无（P5.2 已完成）
- **负责人**：
- **验收标准**：搜索空间覆盖学习率、熵系数、折扣因子

P4.4 不是单个原子问题，按可独立验证的最小单元拆分如下。任务 1 的验收标准已满足，
但它只覆盖"搜索空间定义"，不覆盖"搜索"本身；任务 2-6 均未开始，因此 P4.4 整体未完成。
...
任务 1 未覆盖（留给后续任务，不属于夸大范围）：采样、注入、跟踪、剪枝、分析。

已知遗留（需后续任务处理）：entropy_loss_scale 与 kl_threshold 未声明 grid_points，
网格采样器需要决定对这类连续参数如何取值。
...
3. **P4.4 超参数搜索**：已拆为 6 个原子任务，仅"搜索空间 schema 与产品搜索空间数据"完成；采样器、参数注入、试验跟踪、早停剪枝、结果分析五项均未开始。可复用research_scheduler.py作为试验执行后端（预估剩余 4-5 天）
