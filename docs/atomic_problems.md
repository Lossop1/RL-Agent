...
### P4.4 超参数搜索空间
- **问题**：定义可搜索的超参数及其范围
- **状态**：进行中（拆分为 7 个原子任务，第 1、2、3、4、5、6、7 个完成）
- **阻塞**：无（P5.2 已完成）
- **负责人**：
...
```
grep -cE '^### P[0-9]\.[0-9]' docs/atomic_problems.md                     # 29
grep -E '^- \*\*状态\*\*：' docs/atomic_problems.md | sort | uniq -c      # 19 已完成 / 7 未开始 / 1 部分完成 / 1 单元测试完成 / 1 进行中
```

- **总计**：29 个原子问题（P1.1-P1.5、P2.1-P2.5、P3.1-P3.5、P4.1-P4.5、P5.1-P5.4、P6.1-P6.5）
- **已完成**：19
- **部分完成**：1 (P6.1，步骤 1-3 完成，步骤 4-6 待运行时环境)
- **单元测试完成、待 GPU 验证**：1 (P4.3)
- **进行中**：1 (P4.4，7 个原子任务均已落地，但全链路未接真实训练)
- **未开始**：7 (P6.4、P5.3、P5.4、P3.3、P1.1、P1.2、P1.5)
- **阻塞**：5 个问题被其他问题阻塞

**2026-09-16 订正**：本节此前写的是「总计 30、已完成 22、进行中 1 (P4.3)、未开始 7」。
...
即「30」与「22」不是数出来的。本次改为按条目状态行逐条统计，未开始数由 8 降为 7
是因为 P4.4 本身从「未开始」改记为「进行中」，与总数无关。
