...
WORKFLOWS = {
    "run_tuning_loop": {
        ...
        "steps": [
            {
                "n": 1,
                "do": "Establish ground truth: current per-gate verdict.",
                "tool": "get_acceptance (run_acceptance if stale/absent)",
            ...
            {
                "n": 2,
                "do": "VERIFY the measurement before trusting it (run the verify_measurement workflow).",
                "tool": "get_playbook(task='verify_measurement')",
                "produces": "trusted / rejected",
            ...
            {
                "n": 3,
                "do": "Select the target gate + lever from the profile.",
                "tool": "analyze_acceptance + get_playbook(gate=...)",
                "produces": "hypothesis (gate, lever, expected)",
            ...
            {
                "n": 4,
                "do": "Apply the bounded config edit.",
                "tool": "apply_tuning",
                "produces": "config diff + journal entry (hypothesis logged)",
            ...
            {
                "n": 5,
                "do": "Train from BEST checkpoint to the profile's peak_steps.",
                "tool": "deploy_payload + start_training",
                "produces": "a run driven by the self-heal driver",
            ...
            {
                "n": 6,
                "do": "Re-measure the full benchmark.",
                "tool": "run_acceptance (full battery)",
                "produces": "new verdict",
                "guard": "training STOPPED first; re-run step 2 (verify) on it.",
                "on_fail": "re-measure.",
            },
            {
                "n": 7,
                "do": "Decide keep/rollback; log the outcome.",
                "tool": "compare + rollback_tuning if needed",
                "produces": "journal entry (confirmed/refuted)",
    ...
    "diagnose_training": {
        ...
        "steps": [
            {
                "n": 1,
                "do": "Read the system's own state.",
                "tool": "get_operations_state",
                "guard": "read training_progress (is_resume, from-checkpoint, phase, loaded_payload, regime).",
            },
            {
                "n": 2,
                "do": "Match the pattern to a known signature.",
                "tool": "get_playbook(task='stall')",
                "guard": "the (step-age, process-alive, gpu) pattern matches a profile ops_signature.",
            },
            {
                "n": 3,
                "do": "State the codified diagnosis+action.",
...
def build_playbook(
    *,
    ...
    rules = [
        r.format(
            iter_cost=regime.get("iter_cost", "a long run"),
            peak_steps=regime.get("peak_steps", "the peak"),
        )
    ...
    if picked is None and q:
        ...
        hits = {
            g: v
            for g, v in gl.items()
            if q in g.lower() or q in v.get("symptom", "").lower()
        }
        if hits:
            out["gate_levers"] = hits
    ...
    out["instruction"] = (
        "Robot-AGNOSTIC workflow + this robot's PROFILE. Execute steps with the named tools, "
...
def get_playbook(
    task: str = "", gate: str = "", robot: str = "", *, product_id: str | None = None
) -> dict[str, Any]:
