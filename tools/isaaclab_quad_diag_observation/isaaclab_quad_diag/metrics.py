...
def compute_all_metrics(
    ...
    return compute_metrics(
        df, source=str(record_path), out_dir=out_dir, record_meta=meta
    )
...
def compute_metrics(
    ...
    metrics: dict[str, Any] = {
        ...
        "robustness": compute_robustness_breakdown(
            df, segs, timeline, slices.masks, record_meta
        ),
...
def compute_coverage(
    df: pd.DataFrame, meta: dict[str, Any] | None = None
) -> dict[str, Any]:
    meta = meta or {}
    ...
    if time.notna().any() and "case_id" in df.columns:
        duration = float(
            sum(
                numeric_col(group, "time", np.nan).max()
                - numeric_col(group, "time", np.nan).min()
                for _, group in df.groupby("case_id", dropna=False, sort=False)
            )
        )
    else:
        duration = float(time.max() - time.min()) if time.notna().any() else None
    observed_terrain_types = sorted(
        [
            str(x)
            for x in df.get("terrain_type", pd.Series([], dtype=str)).dropna().unique()
        ]
    )
    observed_dr_levels = sorted(
        [str(x) for x in df.get("dr_level", pd.Series([], dtype=str)).dropna().unique()]
    )
    push_count = int(
        (bool_series(df, "push_event") | bool_series(df, "push_observed")).sum()
    )
    requested_suite = meta.get("requested_suite_config", meta.get("suite", {}))
    executed = meta.get("executed_runtime_config", {})
    terrain_height_sources = (
        sorted(
            [
                str(x)
                for x in df.get("terrain_height_source", pd.Series([], dtype=str))
                .dropna()
                .unique()
            ]
        )
        if "terrain_height_source" in df.columns
        else []
    )
    return {
        "rows": int(len(df)),
        ...
        "episodes": int(
            df[group_key_columns(df, include_segment=False)].drop_duplicates().shape[0]
        )
        ...
        if any(c in df.columns for c in ["case_id", "env_id", "cmd_segment_id"])
        else None,
        "segment_attempts": int(
            df[command_segment_key_columns(df)].drop_duplicates().shape[0]
        )
        if command_segment_key_columns(df)
        else None,
        "episode_segment_fragments": int(
            df[group_key_columns(df)].drop_duplicates().shape[0]
        )
        if group_key_columns(df)
        else None,
        "duration_s": duration,
        "command_modes_observed": sorted(
            [
                str(x)
                for x in df.get(
                    "cmd_target_mode", df.get("__mode", pd.Series([], dtype=str))
                )
                .dropna()
                .unique()
            ]
        ...
        "terrain_levels_observed": sorted(
            [
                float(x)
                for x in pd.to_numeric(
                    df.get("terrain_level", pd.Series([], dtype=float)), errors="coerce"
                )
                .dropna()
                .unique()
            ]
...
def _base_velocity_and_cmd(
    df: pd.DataFrame, command_source: str = "target"
    ...
    if cvx.isna().all():
        cvx = numeric_col(df, "cmd_target_vx", 0.0)
    if cvy.isna().all():
        cvy = numeric_col(df, "cmd_target_vy", 0.0)
    if cwz.isna().all():
        cwz = numeric_col(df, "cmd_target_wz", 0.0)
    return avx, avy, awz, cvx, cvy, cwz


def tracking_stats_for_df(
    df: pd.DataFrame, command_source: str = "target"
) -> dict[str, Any]:
    avx, avy, awz, cvx, cvy, cwz = _base_velocity_and_cmd(
        df, command_source=command_source
    )
    vx_err = (avx - cvx).abs()
    vy_err = (avy - cvy).abs()
    wz_err = (awz - cwz).abs()
...
def _by_bucket(
    df: pd.DataFrame, func: Callable[[pd.DataFrame], dict[str, Any]], cols: list[str]
) -> dict[str, Any]:
    out: dict[str, Any] = {}
...
def compute_tracking(df: pd.DataFrame, masks: dict[str, pd.Series]) -> dict[str, Any]:
    ...
    for slice_name, mask in masks.items():
        ...
        out[slice_name] = {
            ...
            "by_cmd_mode": _by_bucket(
                sub, lambda g: tracking_stats_for_df(g, "target"), ["cmd_target_mode"]
            ),
            "by_cmd_terrain": _by_bucket(
                sub,
                lambda g: tracking_stats_for_df(g, "target"),
                ["cmd_target_mode", "terrain_type"],
            ),
            "by_cmd_dr": _by_bucket(
                sub,
                lambda g: tracking_stats_for_df(g, "target"),
                ["cmd_target_mode", "dr_level"],
            ),
        }
    return out
...
def compute_stand(df: pd.DataFrame, masks: dict[str, pd.Series]) -> dict[str, Any]:
    mode = df.get(
        "cmd_target_mode", df.get("__mode", pd.Series("", index=df.index))
    ).astype(str)
    stand_mask = mode.eq("stand") | (
        (
            numeric_col(df, "cmd_target_vx", 0).abs()
            + numeric_col(df, "cmd_target_vy", 0).abs()
            + numeric_col(df, "cmd_target_wz", 0).abs()
        )
        <= 0.05
    )
    out = {}
    for name in [
        ...
        if len(g) == 0:
            continue
        lin = np.sqrt(
            numeric_col(g, "base_lin_vel_b_x", np.nan) ** 2
            + numeric_col(g, "base_lin_vel_b_y", np.nan) ** 2
        )
        out[name] = {
            "rows": int(len(g)),
            "residual_lin_speed": stats(lin),
            "residual_yaw_rate_abs": stats(
                numeric_col(g, "base_ang_vel_b_z", np.nan).abs()
            ),
            "upright_proxy": stats(
                numeric_col(g, "projected_gravity_b_z", np.nan).abs()
            ),
...
def _four_foot_fraction(df: pd.DataFrame) -> float | None:
    cols = [f"foot_{leg}_contact" for leg in LEGS if f"foot_{leg}_contact" in df.columns]
    if len(cols) < 4:
        return None
...
def compute_gait(df: pd.DataFrame, masks: dict[str, pd.Series]) -> dict[str, Any]:
    ...
    for leg in LEGS:
        ...
        if clr.notna().any():
            ...
            out[f"{leg}_foot_world_z"] = stats(
                numeric_col(g, f"foot_{leg}_pos_w_z", np.nan)
            )
        air = numeric_col(g, f"foot_{leg}_air_time", np.nan)
        if air.notna().any():
            out[f"{leg}_air_time"] = stats(air)
    if duties:
        out["per_leg_duty_range"] = (
            None
            if any(v is None for v in duties.values())
            else float(max(duties.values()) - min(duties.values()))
        )
        if all(k in duties for k in ["FL", "FR"]):
            out["front_left_right_duty_diff"] = _absdiff(duties["FL"], duties["FR"])
        if all(k in duties for k in ["RL", "RR"]):
            out["rear_left_right_duty_diff"] = _absdiff(duties["RL"], duties["RR"])
        diagonal_legs = ["FL", "RR", "FR", "RL"]
        if all(k in duties and duties[k] is not None for k in diagonal_legs):
            out["diagonal_duty_diff"] = abs(
                (duties["FL"] + duties["RR"]) / 2 - (duties["FR"] + duties["RL"]) / 2
            )
    if clear_peaks:
        out["per_leg_clearance_peak_range"] = float(
            max(clear_peaks.values()) - min(clear_peaks.values())
        )
    # Contact count distribution.
    cols = [f"foot_{leg}_contact" for leg in LEGS if f"foot_{leg}_contact" in g.columns]
    if cols:
        cc = sum(bool_series(g, c).astype(int) for c in cols)
        out["contact_count_distribution"] = {
            str(k): float(v)
            for k, v in cc.value_counts(normalize=True).sort_index().items()
        }
        diag = (
            (
                (bool_series(g, "foot_FL_contact") & bool_series(g, "foot_RR_contact"))
                | (
                    bool_series(g, "foot_FR_contact")
                    & bool_series(g, "foot_RL_contact")
                )
            )
            if all(f"foot_{leg}_contact" in g.columns for leg in LEGS)
            else None
        )
        if diag is not None:
    ...
    if len(df) and "cmd_target_mode" in df.columns:
        ...
        for name, g in df.groupby("cmd_target_mode", dropna=False, sort=True):
            ...
            for leg in LEGS:
                ...
                tv.extend(a.tolist())
                fv.extend(b.tolist())
            out["by_cmd_mode"][str(name)] = {
                "touchdown_vz_abs": stats(tv),
                "force_norm": stats(fv),
            }
    return out
...
def compute_posture(df: pd.DataFrame, masks: dict[str, pd.Series]) -> dict[str, Any]:
    ...
    return {
        ...
        "base_height_drop_from_median_max": None
        if h.dropna().empty
        else float(h.dropna().median() - h.dropna().min()),
        "pose_stable_fraction_all": float(
            masks.get("pose_stable", pd.Series(False, index=df.index)).mean()
        )
        if len(df)
        else None,
        "pose_stable_before_first_major_fraction": float(
            masks.get(
                "pose_stable_before_first_major_event", pd.Series(False, index=df.index)
            ).mean()
        )
        if len(df)
        else None,
...
def compute_terrain_breakdown(
    ...
    out: dict[str, Any] = {
        "coverage": {
            "terrain_types_observed": sorted(
                [
                    str(x)
                    for x in df.get("terrain_type", pd.Series([], dtype=str))
                    .dropna()
                    .unique()
                ]
            ),
            "terrain_levels_observed": sorted(
                [
                    float(x)
                    for x in pd.to_numeric(
                        df.get("terrain_level", pd.Series([], dtype=float)),
                        errors="coerce",
                    )
                    .dropna()
                    .unique()
            ...
            "terrain_height_sources_observed": sorted(
                [
                    str(x)
                    for x in df.get("terrain_height_source", pd.Series([], dtype=str))
                    .dropna()
    ...
    if len(seg_df):
        ...
        if all(c in seg_df.columns for c in ["terrain_type", "cmd_target_mode"]):
            for key, g in seg_df.groupby(
                ["terrain_type", "cmd_target_mode"], dropna=False, sort=True
            ):
                out["by_terrain_and_command"][f"terrain={key[0]}/cmd={key[1]}"] = (
                    _segment_progress_stats(g)
                )
    # Per-frame terrain bucket metrics.
    if "terrain_type" in df.columns:
        out["frame_metrics_by_terrain"] = _by_bucket(
            df, _terrain_frame_metrics, ["terrain_type"]
        )
    return out
...
def _segment_progress_stats(seg_df: pd.DataFrame) -> dict[str, Any]:
    return {
        ...
        "raw_progress_along_command": stats(
            seg_df.get("raw_progress_along_command", pd.Series([], dtype=float))
        ),
        "pose_stable_progress_along_command": stats(
            seg_df.get("pose_stable_progress_along_command", pd.Series([], dtype=float))
        ),
        "pre_first_major_event_progress_along_command": stats(
            seg_df.get(
                "pre_first_major_event_progress_along_command",
        ...
        "pose_stable_fraction_all": stats(
            seg_df.get("pose_stable_fraction_all", pd.Series([], dtype=float))
        ),
        "first_major_event_time_since_segment_start_s": stats(
            seg_df.get(
                "first_major_event_time_since_segment_start_s",
                pd.Series([], dtype=float),
            )
        ),
        "reset_observed_segments": int(
            seg_df.get("reset_observed", pd.Series([], dtype=bool)).astype(bool).sum()
        )
        if "reset_observed" in seg_df
        else 0,
        "done_observed_segments": int(
            seg_df.get("done_observed", pd.Series([], dtype=bool)).astype(bool).sum()
        )
        if "done_observed" in seg_df
        else 0,
    }
...
def compute_command_breakdown(segs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for mode, group in seg_df.groupby("cmd_target_mode", dropna=False, sort=True):
        first_major = pd.to_numeric(
            group.get(
                "first_major_event_time_since_segment_start_s", pd.Series(dtype=float)
            ),
            errors="coerce",
        )
        ...
        out[str(mode)] = {
            ...
            "done_attempt_fraction": float(
                group.get("done_observed", pd.Series(False, index=group.index))
                .astype(bool)
                .mean()
            ),
            "reset_attempt_fraction": float(
                group.get("reset_observed", pd.Series(False, index=group.index))
                .astype(bool)
                .mean()
            ),
            "first_major_event_time_s": stats(first_major),
            "pose_stable_fraction_all": stats(
                group.get("pose_stable_fraction_all", pd.Series(dtype=float))
            ),
            "raw_progress_along_command": stats(
                group.get("raw_progress_along_command", pd.Series(dtype=float))
            ),
            "pose_stable_pre_first_major_event_progress_along_command": stats(
                group.get(
                    "pose_stable_pre_first_major_event_progress_along_command",
...
def _terrain_frame_metrics(g: pd.DataFrame) -> dict[str, Any]:
    posture_masks = {
        ...
        "pose_stable_before_first_major_event": pose_stable_mask(g)
        & before_first_major_event_mask(g),
    }
    return {
        "rows": int(len(g)),
...
def compute_robustness_breakdown(
    df: pd.DataFrame,
    segs: list[dict[str, Any]],
    ...
    out: dict[str, Any] = {
        "coverage": {
            "dr_levels_observed": sorted(
                [
                    str(x)
                    for x in df.get("dr_level", pd.Series([], dtype=str))
                    .dropna()
                    .unique()
                ]
            ),
            "dr_columns_observed": [c for c in df.columns if c.startswith("dr_")],
            "push_frames_observed": int(
                (bool_series(df, "push_event") | bool_series(df, "push_observed")).sum()
            ),
            "push_events_observed": len(
                [e for e in timeline if e.get("event_type") == "push_observed"]
            ),
        },
        "by_dr_level": {},
        "by_friction_bucket": {},
    ...
    if "dr_friction" in df.columns:
        temp = df.copy()
        temp["__friction_bucket"] = pd.cut(
            numeric_col(temp, "dr_friction", np.nan),
            bins=[-np.inf, 0.5, 0.8, 1.2, np.inf],
            labels=["<=0.5", "0.5-0.8", "0.8-1.2", ">1.2"],
        )
        out["by_friction_bucket"] = _by_bucket(
            temp, _robustness_frame_metrics, ["__friction_bucket"]
        )
    if "dr_mass" in df.columns:
        temp = df.copy()
        temp["__mass_bucket"] = pd.cut(numeric_col(temp, "dr_mass", np.nan), bins=4)
        out["by_mass_bucket"] = _by_bucket(
            temp, _robustness_frame_metrics, ["__mass_bucket"]
        )
    return out
...
def compute_push_windows(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for idx in df[push].index:
        r = df.loc[idx]
        case = r.get("case_id", 0)
        env = r.get("env_id", 0)
        ep = r.get("episode_id", 0)
        ...
        for name, lo, hi in [
            ("pre_push_0p5s", -0.5, 0),
            ("post_push_0p5s", 0, 0.5),
            ("post_push_1p0s", 0, 1.0),
            ("post_push_2p0s", 0, 2.0),
        ]:
            g = same[
                (numeric_col(same, "time", np.nan) >= et + lo)
                & (numeric_col(same, "time", np.nan) <= et + hi)
            ]
            posture_masks = {
                "pose_stable": pose_stable_mask(g),
                "pose_stable_before_first_major_event": pose_stable_mask(g)
            ...
            out.append(
                {
                    ...
                    "rows": int(len(g)),
                    "posture": compute_posture(g, posture_masks),
                    "tracking": tracking_stats_for_df(g, "target"),
                }
            )
    return out
...
def compute_hardware(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if torque_cols:
        vals = pd.concat(
            [numeric_col(df, c, np.nan).abs() for c in torque_cols], ignore_index=True
        )
    ...
    if util_cols:
        vals = pd.concat(
            [numeric_col(df, c, np.nan).abs() for c in util_cols], ignore_index=True
        )
    ...
    if action_cols and "time" in df.columns:
        ...
        out["action_rate_norm"] = stats(
            rate.replace([np.inf, -np.inf], np.nan), qs=(0.5, 0.9, 0.95, 0.99)
        )
    else:
...
def compute_symmetry(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if duties:
        ...
        out["per_leg_duty_range"] = (
            float(max(duties.values()) - min(duties.values()))
            if len(duties) == 4
            else None
        )
        if all(k in duties for k in ["FL", "FR"]):
            out["front_left_right_duty_diff"] = abs(duties["FL"] - duties["FR"])
        if all(k in duties for k in ["RL", "RR"]):
            out["rear_left_right_duty_diff"] = abs(duties["RL"] - duties["RR"])
    out["mirror_equivariance_error"] = {
        "available": False,
        "reason": "requires live policy adapter and mirror map",
    }
    return out
...
def missing_field_summary(df: pd.DataFrame) -> dict[str, Any]:
    groups = {
        ...
        "local_terrain_height": [
            "foot_FL_terrain_height",
            "foot_FL_clearance_local",
            "terrain_height_source",
        ],
        "touchdown_liftoff": [
            "foot_FL_touchdown",
            "foot_FL_liftoff",
            "foot_FL_touchdown_vz",
        ],
...
def render_summary(metrics: dict[str, Any]) -> str:
    ...
    if not top:
        ...
        for n in top[:12]:
            lines.append(
                f"- [{n.get('priority', '')}] {n.get('type')}: {n.get('message')}"
            )
    lines.append("")
    ...
    lines += [
        ...
        "```json",
        json.dumps(
            metrics.get("event_counts_by_bout_or_instant", {}),
            indent=2,
            ensure_ascii=False,
        ),
        "```",
        ...
        "### Frame Counts",
        "```json",
        json.dumps(metrics.get("event_frame_counts", {}), indent=2, ensure_ascii=False),
        "```",
        "",
    ]
    ...
    # Segment table.
    lines += [
        "## Command Segment Event Summary",
        "",
        "| case | env | ep | seg | mode | terrain | DR | rows | first major event s | events | raw progress | stable pre-event progress | pose-stable frac |",
        "|---:|---:|---:|---:|---|---|---|---:|---:|---|---:|---:|---:|",
    ]
    for s in metrics.get("segment_event_summary", [])[:80]:
        lines.append(
            "| {case_id} | {env_id} | {episode_id} | {cmd_segment_id} | {cmd_target_mode} | {terrain_type} | {dr_level} | {rows} | {fme} | {events} | {raw} | {stablepre} | {frac} |".format(
                **s,
                ...
                stablepre=_format_num(
                    s.get("pose_stable_pre_first_major_event_progress_along_command")
                ),
    ...
    for title, key in [
        ...
        lines += [
            f"## {title}",
            "```json",
            json.dumps(metrics.get(key, {}), indent=2, ensure_ascii=False)[:20000],
            "```",
            "",
        ]
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(
        description="Compute observation-only diagnostics from an IsaacLabQuadDiag record."
    )
