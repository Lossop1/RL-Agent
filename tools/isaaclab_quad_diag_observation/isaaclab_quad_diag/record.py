...
def normalize_dr_cases(value: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(value, dict):
        ...
        if "cases" in value:
            ...
            value = [value]
    return [
        {"level": int(v)} if isinstance(v, (int, float)) else dict(v) for v in value
    ]
...
def main(
...
) -> None:  # pragma: no cover - requires Isaac Lab runtime
    p = argparse.ArgumentParser(
        description="Record Isaac Lab quadruped rollout data in observation-only ILQD schema."
    )
    ...
    p.add_argument(
        "--record-capture",
        default="post_step_terminal_safe",
        choices=["post_step_terminal_safe"],
    )
    ...
    try:
        ...
        raise RuntimeError(
            "ilqd-record must be run inside an Isaac Lab Python environment"
        ) from exc
...
def run_isolated_cases(
    args, unknown: list[str], suite: dict[str, Any], case_count: int
) -> None:
    ...
    child_dirs = [
        cases_dir / f"case_{case_index:03d}" for case_index in range(case_count)
    ]
    ...
    for case_index, child_out in enumerate(child_dirs):
        ...
        cmd.extend(unknown)
        print(
            f"[ILQD] isolated case {case_index + 1}/{case_count} starting", flush=True
        )
        result = subprocess.run(cmd, check=False)
        if result.returncode != 0:
            write_merged_case_outputs(out_dir, child_dirs, args, suite, status="failed")
            raise RuntimeError(
                f"isolated diagnostic case {case_index} exited with code {result.returncode}"
            )
        print(
            f"[ILQD] isolated case {case_index + 1}/{case_count} complete", flush=True
        )
...
def write_merged_case_outputs(
    ...
    with output_csv.open("w", newline="", encoding="utf-8") as dst:
        ...
        for child_dir in child_dirs:
            ...
            executed_cases.extend(
                child_meta.get("executed_runtime_config", {}).get("cases", [])
            )
    ...
    (out_dir / "record_meta.json").write_text(
...
def run_isaaclab_record(args) -> None:  # pragma: no cover - requires Isaac Lab runtime
    ...
    commands = suite.get("commands", []) or [
        {"mode": "stand", "vx": 0, "vy": 0, "wz": 0, "duration": 2.0}
    ]
    ...
    for terrain_case in terrains:
        for dr_case in dr_cases:
            ...
            terrain_apply = apply_terrain_case_best_effort(
                env_cfg, terrain_case, recording_notes
            )
            ...
            command_reset = (
                install_command_conditioned_reset(base)
                if reset_initialization == "command_start"
                else None
            )
            ...
            if args.policy_backend == "skrl":
                ...
                experiment_cfg = load_cfg_from_registry(
                    args.task, "skrl_amp_cfg_entry_point"
                )
            ...
            def act_mean(obs):
                if runner is None:
                    return torch.zeros(
                        (base.num_envs, base.action_space.shape[-1]), device=base.device
                    )
            ...
            foot_names = [
                spec.foot_body_names.get(leg, f"{leg}_foot") for leg in spec.leg_order
            ]
            ...
            if contact_sensor is not None:
                try:
                    ...
                    recording_notes.append(
                        "contact_sensor present but foot bodies were not found"
                    )
            else:
                recording_notes.append(
                    "contact_sensor unavailable; contact/touchdown/slip metrics may be unavailable"
                )
            ...
            dt = float(
                getattr(
                    base,
                    "step_dt",
                    getattr(base, "physics_dt", 0.005)
                    * getattr(base.cfg, "decimation", 1),
                )
            )
            first_cmd = commands[0]
            first_target = torch.tensor(
                [
                    float(first_cmd.get("vx", 0.0)),
                    float(first_cmd.get("vy", 0.0)),
                    float(first_cmd.get("wz", 0.0)),
                ],
                device=base.device,
                dtype=torch.float32,
            )
            ...
            for cmd in commands:
                print(
                    f"[ILQD] segment {seg_id} mode={cmd.get('mode', 'unknown')} starting",
                    flush=True,
                )
                target = torch.tensor(
                    [
                ...
                for k in range(steps):
                    ...
                    common = dict(
                        ...
                        target=target,
                        joint_idx=joint_idx,
                        foot_body_idx=foot_body_idx,
                        foot_contact_idx=foot_contact_idx,
                        legs=spec.leg_order,
                        ...
                        contact_history_valid=contact_history_valid,
                        case_id=case_id,
                    ...
                    terminal_rows = {
                        int(r["env_id"]): r
                        for r in collect_terminal_rows(terminal_capture)
                    }
                    live_ids = [
                        env_id for env_id in range(base.num_envs) if not done[env_id]
                    ]
                    ...
                    missing_terminal_ids = [
                        ...
                        if done[env_id]
                        and env_id not in terminal_rows
                        and env_id not in last_rows_by_env
                    ]
                    reset_rows = (
                        {
                            int(r["env_id"]): r
                            for r in make_rows_from_current_state(
                                **common,
                                ...
                                command_applied=read_env_command_best_effort(
                                    base, target
                                ),
                    ...
                    for env_id in range(base.num_envs):
                        if done[env_id]:
                            ...
                            row["terminal_state_available"] = int(
                                env_id in terminal_rows
                            )
                            row["post_step_state_may_be_after_reset"] = int(
                                env_id not in terminal_rows
                            )
                        else:
...
def install_command_conditioned_reset(
    base,
) -> dict[str, Any]:  # pragma: no cover - requires Isaac Lab runtime
...
def build_columns(n_joints: int, legs: list[str]) -> list[str]:
    cols = [
        ...
        "robot_name",
        "foot_radius",
        "nominal_stand_height",
        "terrain_type_requested",
        "terrain_type",
        "terrain_level",
        ...
        "post_step_state_may_be_after_reset",
        "transition_done_after_action",
        "terminated",
        "truncated",
        "done",
        "reset_observed",
        ...
        "cmd_mode",
        "cmd_segment_id",
        "time_since_command_switch",
        "push_event",
        "push_vector_x",
        "push_vector_y",
        "push_vector_z",
        "push_equivalent_delta_v",
        "base_pos_w_x",
        "base_pos_w_y",
        "base_pos_w_z",
        "base_terrain_height",
    ...
    for i in range(n_joints):
        cols += [
            ...
            f"torque_applied_{i}",
            f"torque_limit_{i}",
            f"torque_utilization_{i}",
            f"action_mean_{i}",
            f"action_applied_{i}",
        ]
    for leg in legs:
        cols += [
            f"foot_{leg}_pos_w_x",
            f"foot_{leg}_pos_w_y",
            f"foot_{leg}_pos_w_z",
            f"foot_{leg}_vel_w_x",
            f"foot_{leg}_vel_w_y",
            f"foot_{leg}_vel_w_z",
            f"foot_{leg}_terrain_height",
            f"foot_{leg}_center_clearance_local",
            f"foot_{leg}_clearance_local",
            f"foot_{leg}_contact",
            f"foot_{leg}_force_w_x",
            f"foot_{leg}_force_w_y",
            f"foot_{leg}_force_w_z",
            f"foot_{leg}_force_norm",
            f"foot_{leg}_normal_force",
            f"foot_{leg}_tangent_force",
            f"foot_{leg}_air_time",
            f"foot_{leg}_stance_time",
            f"foot_{leg}_touchdown",
            f"foot_{leg}_liftoff",
...
def load_evaluation_checkpoint(
    agent, path: str
) -> None:  # pragma: no cover - requires torch/skrl runtime
    import torch
...
def set_external_command(base, target):
    if hasattr(base, "_cmd_target"):
        ...
        raise AttributeError(
            "Cannot set command on this env. Provide a custom recorder adapter."
        )
...
def contact_transition_flags(
    contact_prev: bool, contact_now: bool, history_valid: bool
) -> tuple[int, int]:
    if not history_valid:
        return 0, 0
    return int((not contact_prev) and contact_now), int(
        contact_prev and (not contact_now)
    )
...
def make_rows_from_current_state(**kw) -> list[dict[str, Any]]:  # pragma: no cover
    ...
    base = kw["base"]
    d = kw["robot_data"]
    contact_sensor = kw["contact_sensor"]
    ...
    foot_contact_idx = (
        list(kw["foot_contact_idx"]) if kw["foot_contact_idx"] is not None else None
    )
    ...
    tensors = {
        ...
        "torque": select(
            getattr(d, "applied_torque", getattr(d, "computed_torque", None)), joint_idx
        ),
    ...
    if contact_sensor is not None and foot_contact_idx is not None:
        try:
            tensors["force"] = select(
                contact_sensor.data.net_forces_w, foot_contact_idx
            )
        except Exception:
            tensors["force"] = None
        try:
            tensors["air"] = select(
                contact_sensor.data.current_air_time, foot_contact_idx
            )
        except Exception:
            tensors["air"] = None
    else:
    ...
    if limits is None:
        ...
        if limits_np.ndim == 1:
            arrays["torque_limit"] = np.broadcast_to(
                limits_np[joint_idx], (len(env_ids), len(joint_idx))
            )
        else:
    ...
    if arrays["height_hits"] is not None:
        height_cache = {
            env_id: arrays["height_hits"][index][
                np.isfinite(arrays["height_hits"][index]).all(axis=-1)
            ]
    ...
    target = (
        kw["target"].detach().cpu().numpy()
        if hasattr(kw["target"], "detach")
        else np.asarray(kw["target"])
    )
    rows = []
    for local_i, env_id in enumerate(env_ids):
        ...
        row: dict[str, Any] = {
            ...
            "env_id": env_id,
            "episode_id": int(kw["episode_id"][env_id]),
            "step": int(kw["step"]),
            "time": float(kw["t"]),
            "control_dt": float(kw["dt"]),
            "physics_dt": getattr(base, "physics_dt", np.nan),
            "decimation": getattr(base.cfg, "decimation", np.nan),
            ...
            "terrain_type": actual_terrain_type,
            ...
            "dr_level": str(
                getattr(base, "_dr_level", getattr(base, "dr_level", "unknown"))
            ),
            "capture_stage": kw["capture_stage"],
            "terminal_state_available": 0,
            "post_step_state_may_be_after_reset": 0,
            "transition_done_after_action": 0,
            "terminated": 0,
            "truncated": 0,
            "done": 0,
            "reset_observed": 0,
            "cmd_target_vx": float(target[0]),
            "cmd_target_vy": float(target[1]),
            "cmd_target_wz": float(target[2]),
            "cmd_target_mode": kw["mode"],
            "cmd_vx": cmd_values[0],
            "cmd_vy": cmd_values[1],
            "cmd_wz": cmd_values[2],
            "cmd_mode": derive_mode_from_cmd(*cmd_values),
            "cmd_segment_id": kw["seg_id"],
            "time_since_command_switch": float(kw["time_since_command_switch"]),
            "push_event": int(bool(push)),
            "push_vector_x": push.get("x", np.nan),
            "push_vector_y": push.get("y", np.nan),
            "push_vector_z": push.get("z", np.nan),
            "push_equivalent_delta_v": push.get("delta_v", np.nan),
            "base_pos_w_x": float(pos[0]),
            "base_pos_w_y": float(pos[1]),
            "base_pos_w_z": float(pos[2]),
            "base_terrain_height": terrain_h,
            "base_height_local": float(pos[2]) - terrain_h,
            "base_quat_w": float(quat[0]),
            "base_quat_x": float(quat[1]),
            "base_quat_y": float(quat[2]),
            "base_quat_z": float(quat[3]),
            "base_lin_vel_b_x": float(arrays["lin"][local_i, 0]),
            "base_lin_vel_b_y": float(arrays["lin"][local_i, 1]),
            "base_lin_vel_b_z": float(arrays["lin"][local_i, 2]),
            "base_ang_vel_b_x": float(arrays["ang"][local_i, 0]),
            "base_ang_vel_b_y": float(arrays["ang"][local_i, 1]),
            "base_ang_vel_b_z": float(arrays["ang"][local_i, 2]),
            "projected_gravity_b_x": float(arrays["gravity"][local_i, 0]),
            "projected_gravity_b_y": float(arrays["gravity"][local_i, 1]),
            "projected_gravity_b_z": float(arrays["gravity"][local_i, 2]),
            "dr_mass": dr.get("mass", np.nan),
            "dr_friction": dr.get("friction", np.nan),
            "dr_com_x": dr.get("com_x", np.nan),
            "dr_com_y": dr.get("com_y", np.nan),
        ...
        for k in range(len(joint_idx)):
            ...
            row[f"joint_vel_{k}"] = (
                float(arrays["joint_vel"][local_i, k])
                if arrays["joint_vel"] is not None
                else np.nan
            )
            desired = (
                float(arrays["joint_des"][local_i, k])
                if arrays["joint_des"] is not None
                else np.nan
            )
            row[f"joint_pos_des_{k}"] = desired
            row[f"joint_error_{k}"] = (
                desired - float(q[k]) if np.isfinite(desired) else np.nan
            )
            row[f"torque_applied_{k}"] = (
                float(arrays["torque"][local_i, k])
                if arrays["torque"] is not None
                else np.nan
            )
            limit = (
                float(arrays["torque_limit"][local_i, k])
                if arrays["torque_limit"] is not None
                else np.nan
            )
            ...
            row[f"action_mean_{k}"] = (
                float(arrays["action_mean"][local_i, k])
                if arrays["action_mean"] is not None
                else np.nan
            )
            row[f"action_applied_{k}"] = (
                float(arrays["action_applied"][local_i, k])
                if arrays["action_applied"] is not None
                else np.nan
            )

        for li, leg in enumerate(kw["legs"]):
            fx, fy, fz = [float(x) for x in arrays["foot_pos"][local_i, li]]
            th, thsrc = terrain_height_at_point_best_effort(
                base, env_id, fx, fy, height_cache
            )
            ...
            touchdown, liftoff = contact_transition_flags(
                contact_prev, contact_now, transition_valid
            )
            ...
            row.update(
                {
                    f"foot_{leg}_pos_w_x": fx,
                    f"foot_{leg}_pos_w_y": fy,
                    f"foot_{leg}_pos_w_z": fz,
                    f"foot_{leg}_vel_w_x": vx,
                    f"foot_{leg}_vel_w_y": vy,
                    f"foot_{leg}_vel_w_z": vz,
                    ...
                    f"foot_{leg}_force_w_x": fwx,
                    f"foot_{leg}_force_w_y": fwy,
                    f"foot_{leg}_force_w_z": fwz,
                    f"foot_{leg}_force_norm": fnorm,
                    f"foot_{leg}_normal_force": fwz,
                    f"foot_{leg}_tangent_force": math.sqrt(fwx * fwx + fwy * fwy)
                    if np.isfinite(fwx) and np.isfinite(fwy)
                    else np.nan,
                    f"foot_{leg}_air_time": float(arrays["air"][local_i, li])
                    if arrays["air"] is not None
                    else np.nan,
                    f"foot_{leg}_stance_time": np.nan,
                    f"foot_{leg}_touchdown": touchdown,
                    f"foot_{leg}_liftoff": liftoff,
                    f"foot_{leg}_touchdown_vz": abs(vz)
                    if (touchdown and np.isfinite(vz))
                    else np.nan,
                    f"foot_{leg}_stance_slip_xy": math.sqrt(
                        cp_vx * cp_vx + cp_vy * cp_vy
                    )
            ...
            if (
                thsrc != row["terrain_height_source"]
                and row["terrain_height_source"] == "env_origin_fallback"
            ):
...
def read_terrain_identities_best_effort(
    ...
    try:
        ...
        proportions = np.asarray(
            [generator.sub_terrains[n].proportion for n in names], dtype=float
        )
...
def read_terrain_identity_best_effort(
    base, env_id: int
) -> tuple[str, float]:  # pragma: no cover
    return read_terrain_identities_best_effort(base, [env_id])[env_id]


def apply_terrain_case_best_effort(
    env_cfg, terrain_case: dict[str, Any], notes: list[str]
    ...
    try:
        ...
        if requested in ["stairs_up", "stairs_down"]:
            notes.append(
                f"{requested} uses the actual 'stairs' terrain; ascent/descent is determined from observed motion"
            )
        return {
            "applied": True,
...
def force_terrain_level_best_effort(
    base, terrain_case: dict[str, Any], notes: list[str]
...
def apply_dr_case_best_effort(
    env_cfg, dr_case: dict[str, Any], notes: list[str]
) -> dict[str, Any]:  # pragma: no cover
    ...
    try:
        ...
        reason = (
            f"DR case could not be applied: {dr_case} ({type(exc).__name__}: {exc})"
        )
        notes.append(reason)
        return {"applied": False, "reason": reason}


def refresh_dr_snapshot_best_effort(
    base, notes: list[str], env_ids=None
) -> None:  # pragma: no cover
    count = int(base.num_envs)
    ids = (
        np.arange(count, dtype=int)
        if env_ids is None
        else np.asarray(env_ids, dtype=int)
    )
    ...
    try:
        ...
        mats_np = (
            mats.detach().cpu().numpy() if hasattr(mats, "detach") else np.asarray(mats)
        )
    ...
    try:
        ...
        coms_np = (
            coms.detach().cpu().numpy() if hasattr(coms, "detach") else np.asarray(coms)
        )
    ...
    cache["latency"][ids] = float(getattr(base.cfg, "action_delay_steps", 0)) * float(
        getattr(
            base,
            "step_dt",
            getattr(base.cfg, "dt", 0.0) * getattr(base.cfg, "decimation", 1),
        )
    )
...
def maybe_apply_push_best_effort(
    ...
    for push_index, p in enumerate(pushes):
        ...
        try:
            ...
            vector = p.get(
                "vector", [p.get("x", 0.0), p.get("y", 0.0), p.get("z", 0.0)]
            )
            ...
            velocity = torch.cat(
                [
                    base.robot.data.root_lin_vel_w[ids],
                    base.robot.data.root_ang_vel_w[ids],
                ],
                dim=-1,
            ).clone()
            ...
            notes.append(
                f"push request could not be applied: {p} ({type(exc).__name__}: {exc})"
            )
    return result
